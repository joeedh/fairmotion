...
export function pack_vec3(
  data: number[],
  vec: { [k: number]: number | undefined },
  lendian = false
) {
...
export function pack_dataref(data: number[], b: DataBlock | undefined, lendian = false) {
  if (b != undefined) {
    ...
    if (b.lib_lib != undefined) pack_int(data, b.lib_lib.id, lendian);
    else pack_int(data, -1, lendian);
  } else {
...
export function test_str_packers() {
  function static_string_test() {
    ...
    if (arr.length != 9) throw new UnitTestError("Bad length " + arr.length.toString());
    ...
    if (str2 != "12345678") throw new UnitTestError("Bad truncation");
  }
...
export function unpack_array<T>(
  data: DataView,
  uctx: unpack_ctx,
...
export function unpack_garray<T>(
  data: DataView,
  uctx: unpack_ctx,
  unpacker: (data: DataView, uctx: unpack_ctx) => T
) {
...
window.api_exec = function api_exec(
  path: string,
  netjob: NetJob,
  mode?: string,
  ...
  const error: Function =
    netjob.error ??
    function (netjob: NetJob, owner: object, msg: string) {
  ...
  if (mode != "GET") req.setRequestHeader("Content-type", mime);
  ...
  req.onprogress = function (evt: ProgressEvent) {
    ...
    if (DEBUG.netio) console.log("perc", perc, netjob.status);

    if (netjob.status) netjob.status(netjob, netjob.owner, netjob.status_data);
  };
  ...
  req.onreadystatechange = function () {
    if (req.readyState == 4 && req.status >= 200 && req.status <= 300) {
      ...
      if (DEBUG.netio) console.log("headers:", netjob.headers);
...
window.AuthSessionGen = function* AuthSessionGen(
  job: NetJob,
  user: string,
  password: string,
  ...
  if (job.finish != undefined) job.finish(job, job.owner);
};

window.auth_session = function auth_session(
  user: string,
  password: string,
  finish?: Function,
  error?: Function,
...
window.call_api = function call_api<Args>(
  ...
  var promise = new Promise(function (accept, reject) {
    ...
    function finish(...args: unknown[]) {
      if (finishcb != undefined) finishcb(...args);

      accept(args[0]);
    }

    function error(...args: unknown[]) {
      if (errorcb != undefined) errorcb(...args);
      if (reject != undefined) reject(args[0]);
    }
...
window.upload_file = function* upload_file(job: NetJob, args) {
  ...
  if (DEBUG.netio) console.log(job.value);
  ...
  if (DEBUG.netio) console.log("beginning upload", ilen);
  for (var i = 0; i < ilen; i++) {
    if (DEBUG.netio) console.log("Uploading chunk " + (i + 1) + " of " + ilen);

    var url = url2 + "&uploadToken=" + upload_token;
    var size = i == ilen - 1 ? len % csize : csize;

    if (DEBUG.netio) console.log(i * csize, size, data);
    ...
    if (DEBUG.netio) console.log(headers["Content-Range"], size, c, chunk);
