...
import type { FileData, FileErrorCallback, OpenFileCallback } from "./fileapi.js";
...
export function open_file(
  ...
  chrome.fileSystem.chooseEntry(params, function (readOnlyEntry: ChromeFileEntry) {
    ...
    if (set_current_file) current_chromeapp_file = readOnlyEntry;
...
export function save_file(
  ...
  function chooseFile() {
    ...
    chrome.fileSystem.chooseEntry(params, function (writableFileEntry) {
      ...
      if (set_current_file) current_chromeapp_file = writableFileEntry;
  ...
  function error(...args: unknown[]) {
    ...
    if (error_cb != undefined) error_cb(args[0]);
  }
