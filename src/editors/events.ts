...
export class MyKeyboardEvent {
  ...
  constructor(code : number, shift : boolean = false, ctrl : boolean = false,
              alt : boolean = false) {
    this.keyCode = code;
    this.shiftKey = shift;
    this.ctrlKey = ctrl;
...
export function swap_next_mouseup_event(button : number) {
...
export function ignore_next_mouseup_event(button : number) {
...
export class EventHandler {
  ...
  /* NOTE: both forwarders below pass a bare `event`, which is not a
     parameter here -- they read window.event and drop their real arguments. */
  //resize events aren't modal
  _on_resize(newsize : number[])
  {
    this.on_resize(event);
  }

  _on_pan(pan : number[], last_pan : number[])
  {
...
export class VelocityPan extends EventHandler {
  ...
  start(start_mpos : number[], last_mpos : number[], owner,
        push_modal_func : (handler : EventHandler) => void,
        pop_modal_func : () => void) {
