...
es6_module_define('nacl_api', ["spline_types", "toolops_api", "spline_base", "typedwriter", "spline_math_hermite", "solver", "ajax"], function _nacl_api_module(_es6_module) {
  "use strict";
  ...
  function gen_draw_cache(postMessage, status, spline) {
    ...
    function _generator_iter() {
      this.scope = {__iter_s_1: undefined, s_1: undefined, postMessage_0: postMessage, data_1: undefined, msgid_1: undefined, tot_18: undefined, upack_18: undefined, getfloat_18: undefined, i_18: undefined, eidmap_18: undefined, spline_0: spline, dview_18: undefined, ret_18: undefined, getint_18: undefined, getdouble_18: undefined, endian_1: undefined, status_0: status}
...
es6_module_define('transform', ["dopesheet_transdata", "mathlib", "events", "toolops_api", "multires_transdata", "nacl_api", "transdata", "toolprops", "selectmode", "spline_types"], function _transform_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  ...
  var TransSplineVert=_ESClass("TransSplineVert", [_ESClass.static(function apply(ctx, td, item, mat, w) {
    ...
    if (w==0.0)
      return ;
    $co_hrFN_apply.load(item.start_data);
    $co_hrFN_apply[2] = 0.0;
    $co_hrFN_apply.multVecMatrix(mat);
    v.load($co_hrFN_apply).sub(item.start_data).mulScalar(w).add(item.start_data);
    ...
    if (item.data.hidden)
      return ;
    $co_TbUI_aabb.load(item.data);
    $co_TbUI_aabb[2] = 0.0;
    minmax.minmax($co_TbUI_aabb);
  ...
  var $co_hrFN_apply=new Vector3();
  var $co_TbUI_aabb=new Vector3();
  ...
  var $min1_ID_s_post_mousemove;
  var $min2_PBOz_post_mousemove;
  var $max1_0Amm_post_mousemove;
  var $max2_WLIX_post_mousemove;
  var TransformOp=_ESClass("TransformOp", ToolOp, [function TransformOp(start_mpos, datamode) {
    ...
    $min1_ID_s_post_mousemove.load(minmax.min);
    $max1_0Amm_post_mousemove.load(minmax.max);
    minmax.reset();
    ...
    if (do_last) {
        for (var i=0; i<2; i++) {
            $min2_PBOz_post_mousemove[i] = Math.min($min1_ID_s_post_mousemove[i], minmax.min[i]);
            $max2_WLIX_post_mousemove[i] = Math.max($max1_0Amm_post_mousemove[i], minmax.max[i]);
        }
    }
    else {
      $min2_PBOz_post_mousemove.load(minmax.min), $max2_WLIX_post_mousemove.load(minmax.max);
    }
    var found=false;
    ...
    if (ctx.spline.resolve==0) {
        if (force_solve&&!ctx.spline.solving) {
            redraw_viewport($min2_PBOz_post_mousemove, $max2_WLIX_post_mousemove, undefined, !this2.first_viewport_redraw);
        }
        else 
          if (force_solve) {
  ...
  var $min1_ID_s_post_mousemove=new Vector3();
  var $min2_PBOz_post_mousemove=new Vector3();
  var $max1_0Amm_post_mousemove=new Vector3();
  var $max2_WLIX_post_mousemove=new Vector3();
  _es6_module.add_class(TransformOp);
  TransformOp = _es6_module.add_export('TransformOp', TransformOp);
  var $start_4PDE_on_mousemove;
  var $mat_sCD9_exec;
  var $off_4rV7_on_mousemove;
  var TranslateOp=_ESClass("TranslateOp", TransformOp, [function TranslateOp(user_start_mpos, datamode) {
    ...
    var ctx=this.modal_ctx;
    var td=this.transdata;
    $start_4PDE_on_mousemove.load(md.start_mpos);
    $off_4rV7_on_mousemove.load(md.mpos);
    ctx.view2d.unproject($start_4PDE_on_mousemove);
    ctx.view2d.unproject($off_4rV7_on_mousemove);
    $off_4rV7_on_mousemove.sub($start_4PDE_on_mousemove);
    this.inputs.translation.set_data($off_4rV7_on_mousemove);
    this.exec(ctx);
    ...
    $mat_sCD9_exec.makeIdentity();
    $mat_sCD9_exec.translate(off[0], off[1], 0);
    ...
    while (1) {
      ...
      d = __ival_d.value;
      d.type.apply(ctx, td, d, $mat_sCD9_exec, d.w);
    }
    this.update(ctx);
  ...
  var $start_4PDE_on_mousemove=new Vector3();
  var $mat_sCD9_exec=new Matrix4();
  var $off_4rV7_on_mousemove=new Vector3();
  _es6_module.add_class(TranslateOp);
  TranslateOp = _es6_module.add_export('TranslateOp', TranslateOp);
  var $scale_29cZ_on_mousemove;
  var $mat_F2Ay_exec;
  var $off_3dBc_on_mousemove;
  var ScaleOp=_ESClass("ScaleOp", TransformOp, [function ScaleOp(user_start_mpos, datamode) {
    ...
    var ctx=this.modal_ctx;
    var td=this.transdata;
    var l1=$off_3dBc_on_mousemove.load(md.mpos).sub(td.scenter).vectorLength();
    var l2=$off_3dBc_on_mousemove.load(md.start_mpos).sub(td.scenter).vectorLength();
    $scale_29cZ_on_mousemove[0] = $scale_29cZ_on_mousemove[1] = l1/l2;
    $scale_29cZ_on_mousemove[2] = 1.0;
    this.inputs.scale.set_data($scale_29cZ_on_mousemove);
    this.exec(ctx);
    this.post_mousemove(event);
  }, function exec(ctx) {
    var td=this.modal_running ? this.transdata : this.ensure_transdata(ctx);
    ...
    var cent=td.center;
    $mat_F2Ay_exec.makeIdentity();
    $mat_F2Ay_exec.translate(cent[0], cent[1], 0);
    $mat_F2Ay_exec.scale(scale[0], scale[1], scale[2]);
    $mat_F2Ay_exec.translate(-cent[0], -cent[1], 0);
    var __iter_d=__get_iter(td.data);
    var d;
    while (1) {
      ...
      d = __ival_d.value;
      d.type.apply(ctx, td, d, $mat_F2Ay_exec, d.w);
    }
    this.update(ctx);
    if (!this.modal_running) {
        ctx.frameset.on_ctx_update(ctx);
  ...
  var $scale_29cZ_on_mousemove=new Vector3();
  var $mat_F2Ay_exec=new Matrix4();
  var $off_3dBc_on_mousemove=new Vector3();
  _es6_module.add_class(ScaleOp);
  ScaleOp = _es6_module.add_export('ScaleOp', ScaleOp);
  var $off_oLuu_on_mousemove;
  var $mat_SuvR_exec;
  var RotateOp=_ESClass("RotateOp", TransformOp, [function RotateOp(user_start_mpos, datamode) {
    ...
    var td=this.transdata;
    this.reset_drawlines();
    var l1=$off_oLuu_on_mousemove.load(md.mpos).sub(td.scenter).vectorLength();
    var l2=$off_oLuu_on_mousemove.load(md.start_mpos).sub(td.scenter).vectorLength();
    ...
    var td=this.modal_running ? this.transdata : this.ensure_transdata(ctx);
    var cent=td.center;
    $mat_SuvR_exec.makeIdentity();
    $mat_SuvR_exec.translate(cent[0], cent[1], 0);
    $mat_SuvR_exec.rotate(this.inputs.angle.data, 0, 0, 1);
    $mat_SuvR_exec.translate(-cent[0], -cent[1], 0);
    var __iter_d=__get_iter(td.data);
    var d;
    while (1) {
      ...
      d = __ival_d.value;
      d.type.apply(ctx, td, d, $mat_SuvR_exec, d.w);
    }
    this.update(ctx);
    if (!this.modal_running) {
        ctx.frameset.on_ctx_update(ctx);
  ...
  var $off_oLuu_on_mousemove=new Vector3();
  var $mat_SuvR_exec=new Matrix4();
  _es6_module.add_class(RotateOp);
  RotateOp = _es6_module.add_export('RotateOp', RotateOp);
});
es6_module_define('transform_ops', ["spline_types", "nacl_api", "selectmode", "dopesheet_transdata", "transform", "toolops_api", "mathlib", "multires_transdata", "events", "toolprops", "transdata"], function _transform_ops_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
...
es6_module_define('spline_selectops', ["toolprops", "spline_types", "toolops_api", "spline_draw", "animdata"], function _spline_selectops_module(_es6_module) {
  "use strict";
  var $_mh;
  var $_swapt;
...
es6_module_define('spline_createops', ["spline", "toolprops", "spline_types", "toolops_api", "spline_editops"], function _spline_createops_module(_es6_module) {
  var ToolOp=es6_import_item(_es6_module, 'toolops_api', 'ToolOp');
  var SplineFlags=es6_import_item(_es6_module, 'spline_types', 'SplineFlags');
  var EnumProperty=es6_import_item(_es6_module, 'toolprops', 'EnumProperty');
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
...
es6_module_define('spline_editops', ["animdata", "struct", "spline_draw", "frameset", "spline_types", "toolprops", "toolops_api", "spline"], function _spline_editops_module(_es6_module) {
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
  var FloatProperty=es6_import_item(_es6_module, 'toolprops', 'FloatProperty');
  var CollectionProperty=es6_import_item(_es6_module, 'toolprops', 'CollectionProperty');
  var BoolProperty=es6_import_item(_es6_module, 'toolprops', 'BoolProperty');
...
es6_module_define('spline_layerops', ["toolprops", "toolops_api", "spline_types", "spline_editops", "spline"], function _spline_layerops_module(_es6_module) {
  var ToolOp=es6_import_item(_es6_module, 'toolops_api', 'ToolOp');
  var UndoFlags=es6_import_item(_es6_module, 'toolops_api', 'UndoFlags');
  var ToolFlags=es6_import_item(_es6_module, 'toolops_api', 'ToolFlags');
  var SplineFlags=es6_import_item(_es6_module, 'spline_types', 'SplineFlags');
...
es6_module_define('multires_ops', ["spline_multires", "spline_types", "spline_editops", "toolops_api", "toolprops", "spline", "J3DIMath", "spline_draw"], function _multires_ops_module(_es6_module) {
  ...
  var CreateMResPoint=_ESClass("CreateMResPoint", SplineLocalToolOp, [function CreateMResPoint(seg, co) {
    ...
    if (cp!=undefined) {
        ...
        $vec_QjQa_exec.zero().load(co).sub(cp[0]);
        var n=seg.normal(s);
        t*=Math.sign(n.dot($vec_QjQa_exec));
        p.offset[0] = $vec_QjQa_exec[0];
        p.offset[1] = $vec_QjQa_exec[1];
    }
    else {
...
es6_module_define('multires_selectops', ["toolprops", "spline_editops", "toolops_api", "spline_multires", "spline_types", "J3DIMath", "spline_draw", "spline"], function _multires_selectops_module(_es6_module) {
  "use strict";
  es6_import(_es6_module, 'J3DIMath');
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
  var FloatProperty=es6_import_item(_es6_module, 'toolprops', 'FloatProperty');
...
es6_module_define('multires_transdata', ["selectmode", "transdata", "spline_multires", "mathlib"], function _multires_transdata_module(_es6_module) {
  "use strict";
  var SelMask=es6_import_item(_es6_module, 'selectmode', 'SelMask');
  var compose_id=es6_import_item(_es6_module, 'spline_multires', 'compose_id');
  var decompose_id=es6_import_item(_es6_module, 'spline_multires', 'decompose_id');
  ...
  var TransDataItem=es6_import_item(_es6_module, 'transdata', 'TransDataItem');
  var $co_fo9N_apply;
  var $co_voqA_calc_draw_aabb;
  var $co2_rzll_calc_draw_aabb;
  var $co_UUPF_aabb;
  var MResTransData=_ESClass("MResTransData", TransDataType, [_ESClass.static(function gen_data(ctx, td, data) {
    ...
    if (w==0.0)
      return ;
    $co_fo9N_apply.load(item.start_data);
    $co_fo9N_apply[2] = 0.0;
    $co_fo9N_apply.multVecMatrix(mat);
    $co_fo9N_apply.sub(item.start_data).mulScalar(w).add(item.start_data);
    p[0] = $co_fo9N_apply[0];
    p[1] = $co_fo9N_apply[1];
    ...
    $co_voqA_calc_draw_aabb.zero();
    var pad=15;
    function do_minmax(co) {
      $co2_rzll_calc_draw_aabb[0] = co[0]-pad;
      $co2_rzll_calc_draw_aabb[1] = co[1]-pad;
      minmax.minmax($co2_rzll_calc_draw_aabb);
      $co2_rzll_calc_draw_aabb[0]+=pad*2.0;
      $co2_rzll_calc_draw_aabb[1]+=pad*2.0;
      minmax.minmax($co2_rzll_calc_draw_aabb);
    }
    var spline=ctx.spline;
    for (var i=0; i<td.data.length; i++) {
        ...
        $co_voqA_calc_draw_aabb[0] = t.data[0];
        $co_voqA_calc_draw_aabb[1] = t.data[1];
        do_minmax($co_voqA_calc_draw_aabb);
        $co_voqA_calc_draw_aabb[0]-=t.data.offset[0];
        $co_voqA_calc_draw_aabb[1]-=t.data.offset[1];
        do_minmax($co_voqA_calc_draw_aabb);
    }
  }), _ESClass.static(function aabb(ctx, td, item, minmax, selected_only) {
    $co_UUPF_aabb.zero();
    for (var i=0; i<td.data.length; i++) {
        ...
        if (t.type!==MResTransData)
          continue;
        $co_UUPF_aabb[0] = t.data[0];
        $co_UUPF_aabb[1] = t.data[1];
        minmax.minmax($co_UUPF_aabb);
    }
  }), function MResTransData() {
    TransDataType.apply(this, arguments);
  }]);
  var $co_fo9N_apply=new Vector3();
  var $co_voqA_calc_draw_aabb=new Vector3();
  var $co2_rzll_calc_draw_aabb=[0, 0, 0];
  var $co_UUPF_aabb=new Vector3();
...
es6_module_define('UIElement', ["events", "toolprops", "mathlib"], function _UIElement_module(_es6_module) {
  ...
  var $pos2_GdfN_inrect_2d_button=new Vector2();
  var $size2_1r_z_inrect_2d_button=new Vector2();
  function inrect_2d_button(p, pos, size) {
    if (g_app_state.was_touch) {
        $pos2_GdfN_inrect_2d_button.load(pos);
        $size2_1r_z_inrect_2d_button.load(size);
        $pos2_GdfN_inrect_2d_button.subScalar(fuzzy_ui_press_hotspot);
        $size2_1r_z_inrect_2d_button.addScalar(fuzzy_ui_press_hotspot*2.0);
        return inrect_2d(p, $pos2_GdfN_inrect_2d_button, $size2_1r_z_inrect_2d_button);
    }
    else {
  ...
  var $empty_arr_h1px_get_keymaps;
  var $pos_qJiD_get_abs_pos;
  var $ret_yHvF_get_min_size;
  var UIElement=_ESClass("UIElement", EventHandler, [function UIElement(ctx, path, pos, size) {
    ...
    return $empty_arr_h1px_get_keymaps;
    ...
    $pos_qJiD_get_abs_pos[0] = this.pos[0];
    $pos_qJiD_get_abs_pos[1] = this.pos[1];
    var p=this.parent;
    while (p!=undefined) {
      $pos_qJiD_get_abs_pos[0]+=p.pos[0];
      $pos_qJiD_get_abs_pos[1]+=p.pos[1];
      p = p.parent;
    }
    return $pos_qJiD_get_abs_pos;
  ...
  var $empty_arr_h1px_get_keymaps=[];
  var $pos_qJiD_get_abs_pos=[0, 0];
  var $ret_yHvF_get_min_size=[1, 1];
...
es6_module_define('UICanvas', ["mathlib", "UIElement"], function _UICanvas_module(_es6_module) {
  ...
  var $ret_3zln_get_2d_canvas={}
  function get_2d_canvas() {
    if ($ret_3zln_get_2d_canvas.canvas==undefined) {
        $ret_3zln_get_2d_canvas.canvas = document.getElementById("canvas2d");
        $ret_3zln_get_2d_canvas.ctx = _canvas2d_ctx;
    }
    return $ret_3zln_get_2d_canvas;
  }
  get_2d_canvas = _es6_module.add_export('get_2d_canvas', get_2d_canvas);
  window.get_2d_canvas = get_2d_canvas;
  var $ret_6VlS_get_2d_canvas_2={}
  function get_2d_canvas_2() {
    if ($ret_6VlS_get_2d_canvas_2.canvas==undefined) {
        $ret_6VlS_get_2d_canvas_2.canvas = document.getElementById("canvas2d_work");
        $ret_6VlS_get_2d_canvas_2.ctx = _canvas2d_ctx_2;
    }
    return $ret_6VlS_get_2d_canvas_2;
  }
  ...
  var $temp_layer_idgen_MkNr_push_layer;
  var $black_17rv_quad;
  var $grads_GJJi_quad;
  var $mid__wrM_colorfield;
  var $cache_AusC_box1;
  var $v1_zyZ0_box1;
  var $v3_fu7L_box1;
  var $pairs_QGhf_box1;
  ...
  var UICanvas=_ESClass("UICanvas", [function UICanvas(viewport) {
    ...
    var canvas=document.createElement("canvas");
    canvas.id = "_temp_canvas2d_"+($temp_layer_idgen_MkNr_push_layer++);
    document.body.appendChild(canvas);
    canvas.style["position"] = "absolute";
    canvas.style["left"] = "0px";
    canvas.style["top"] = "0px";
    ...
    $mid__wrM_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        if (color[i]==0.0)
          $mid__wrM_colorfield[i] = 0.0;
        else 
          $mid__wrM_colorfield[i] = color[i];
    }
    var color2=this._css_color($mid__wrM_colorfield);
    $mid__wrM_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        $mid__wrM_colorfield[i] = (color[i]*3.0-1.0)/4.0;
    }
    var midclr=this._css_color($mid__wrM_colorfield);
    $mid__wrM_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        $mid__wrM_colorfield[i] = 0.5+color[i]*0.5;
    }
    var smidclr=this._css_color($mid__wrM_colorfield);
    $mid__wrM_colorfield[3] = 0.0;
    for (var i=0; i<3; i++) {
        $mid__wrM_colorfield[i] = color[i];
    }
    var zerocolor=this._css_color($mid__wrM_colorfield);
    ...
    if (!(hash in $cache_AusC_box1)) {
        ...
        p2.reverse();
        $cache_AusC_box1[hash] = [p1, p2, points];
    }
    var cp=$cache_AusC_box1[hash];
    var p1=cp[0];
    var p2=cp[1];
    var points=cp[2];
    var plen=p1.length;
    ...
    if (!outline_only) {
        for (var i=0; i<p1.length-1; i++) {
            ...
            var i3=i+1+plen*2;
            var i4=i+1;
            $v1_zyZ0_box1[0] = p1[i][0]+x;
            $v1_zyZ0_box1[1] = p1[i][1]+y;
            $v1_zyZ0_box1[2] = p1[i][2];
            
            $v2_FOfA_box1[0] = p2[i][0]+x;
            $v2_FOfA_box1[1] = p2[i][1]+y;
            $v2_FOfA_box1[2] = p2[i][2];
            
            $v3_fu7L_box1[0] = p2[i+1][0]+x;
            $v3_fu7L_box1[1] = p2[i+1][1]+y;
            $v3_fu7L_box1[2] = p2[i+1][2];
            
            $v4_GSm3_box1[0] = p1[i+1][0]+x;
            $v4_GSm3_box1[1] = p1[i+1][1]+y;
            $v4_GSm3_box1[2] = p1[i+1][2];
            
            this.quad($v1_zyZ0_box1, $v2_FOfA_box1, $v3_fu7L_box1, $v4_GSm3_box1, color(i1), color(i2), color(i3), color(i4));
        }
    }
    var lines=[];
    var colors=[];
    for (var i=0; i<points.length; i++) {
        $v1_zyZ0_box1[0] = points[(i+1)%points.length][0]+x;
        $v1_zyZ0_box1[1] = points[(i+1)%points.length][1]+y;
        $v1_zyZ0_box1[2] = points[(i+1)%points.length][2];
        
        $v2_FOfA_box1[0] = points[i][0]+x;
        $v2_FOfA_box1[1] = points[i][1]+y;
        $v2_FOfA_box1[2] = points[i][2];
        
        if ($pairs_QGhf_box1.length<=i) {
            $pairs_QGhf_box1.push([[0, 0], [0, 0]]);
        }
        $pairs_QGhf_box1[i][0][0] = (($_mh = objcache.array(2)), ($_mh[0] = ($v1_zyZ0_box1[0])), ($_mh[1] = ($v1_zyZ0_box1[1])), ($_mh[2] = (0)), $_mh);
        $pairs_QGhf_box1[i][0][1] = (($_mh = objcache.array(2)), ($_mh[0] = ($v2_FOfA_box1[0])), ($_mh[1] = ($v2_FOfA_box1[1])), ($_mh[2] = (0)), $_mh);
        lines.push($pairs_QGhf_box1[i][0]);
        $pairs_QGhf_box1[i][1][0] = color((i+1)%points.length);
        $pairs_QGhf_box1[i][1][1] = color(i);
        colors.push($pairs_QGhf_box1[i][1]);
    }
    ...
    for (var i=0; i<lines.length; i++, ly+=12) {
        ...
        var m=this.transmat.$matrix;
        $pos_JOgP_text[0] = m.m41+v[0][0]+pos1[0];
        $pos_JOgP_text[1] = canvas.height-(m.m42+v[0][1]+pos1[1]+ly);
        $pos_JOgP_text[2] = 0;
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.rotate(rot);
        if (rot!=0) {
        ...
        rot2d($pos_JOgP_text, -rot);
        $pos_JOgP_text[1] = canvas.height-$pos_JOgP_text[1];
        if (color==undefined)
          color = [0, 0, 0, 1];
        ...
        var x=$pos_JOgP_text[0], y=canvas.height-($pos_JOgP_text[1]);
  ...
  var $temp_layer_idgen_MkNr_push_layer=0;
  var $black_17rv_quad=[0, 0, 0, 1];
  var $grads_GJJi_quad={}
  var $mid__wrM_colorfield=[0, 0, 0, 0.5];
  var $cache_AusC_box1={}
  var $v1_zyZ0_box1=new Vector3();
  var $v3_fu7L_box1=new Vector3();
  var $pairs_QGhf_box1=[];
  var $pos_JOgP_text=[0, 0, 0];
  var $v2_FOfA_box1=new Vector3();
  var $v4_GSm3_box1=new Vector3();
...
es6_module_define('UIFrame', ["events", "UIElement", "J3DIMath", "mathlib"], function _UIFrame_module(_es6_module) {
  var aabb_isect_2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_2d');
  var inrect_2d=es6_import_item(_es6_module, 'mathlib', 'inrect_2d');
  es6_import(_es6_module, 'J3DIMath');
  var PackFlags=es6_import_item(_es6_module, 'UIElement', 'PackFlags');
  ...
  var $pos_LIlr__find_active;
  var $zero_UPIl_build_draw_old;
  var UIFrame=_ESClass("UIFrame", UIElement, [function UIFrame(ctx, canvas, path, pos, size) {
    ...
    for (var i=this.children.length-1; i>=0; i--) {
        ...
        $pos_LIlr__find_active[0] = c.pos[0], $pos_LIlr__find_active[1] = c.pos[1];
        if (c.state&UIFlags.HAS_PAN) {
        }
        if (inrect_2d(mpos, $pos_LIlr__find_active, c.size)) {
            found = true;
  ...
  var $pos_LIlr__find_active=[0, 0];
  var $zero_UPIl_build_draw_old=[0, 0];
  _es6_module.add_class(UIFrame);
  UIFrame = _es6_module.add_export('UIFrame', UIFrame);
});
es6_module_define('UIPack', ["UIElement", "UIFrame", "mathlib", "UIWidgets", "toolprops"], function _UIPack_module(_es6_module) {
...
es6_module_define('icon', [], function _icon_module(_es6_module) {
  ...
  var IconManager=_ESClass("IconManager", [function IconManager(gl, sheet_path, imgsize, iconsize) {
    ...
    x*=cellsize[0];
    y*=cellsize[1];
    $ret_OBJ8_enum_to_xy[0] = x;
    $ret_OBJ8_enum_to_xy[1] = y;
    return $ret_OBJ8_enum_to_xy;
  ...
  var $ret_OBJ8_enum_to_xy=[0, 0];
  _es6_module.add_class(IconManager);
  IconManager = _es6_module.add_export('IconManager', IconManager);
  var icon_vshader="\n\n";
  var icon_fshader="\n";
});
es6_module_define('UIWidgets', ["events", "UIElement", "UIFrame", "units", "toolprops", "mathlib"], function _UIWidgets_module(_es6_module) {
  var $_mh;
  var $_swapt;
  var UIFrame=es6_import_item(_es6_module, 'UIFrame', 'UIFrame');
  ...
  var $pos_v9bJ_build_draw;
  var $high_clr_JQ3J_build_draw;
  var $size_IMSU_build_draw;
  var $inset_clr_Y_iP_build_draw;
  var UIButtonIcon=_ESClass("UIButtonIcon", UIButton, [function UIButtonIcon(ctx, text, icon, pos, size, path, callback, hint, use_small_icon) {
    ...
    if (this.icon==-1) {
        if (!(this.state&UIFlags.ENABLED))
          ...
          if (this.clicked)
          canvas.box($pos_v9bJ_build_draw, $size_IMSU_build_draw, uicolors["IconInv"]);
        else 
          if (this.state&UIFlags.HIGHLIGHT)
          canvas.box($pos_v9bJ_build_draw, $size_IMSU_build_draw, uicolors["HighlightIcon"]);
        else 
          canvas.box($pos_v9bJ_build_draw, this.size, uicolors["IconBox"]);
        return ;
    }
    var pad=this.pad;
    var isize=this.small_icon ? canvas.iconsheet16.cellsize : canvas.iconsheet.cellsize;
    if (isize[0]>this.size[0])
      $pos_v9bJ_build_draw[0] = 1;
    else 
      $pos_v9bJ_build_draw[0] = 1;
    $pos_v9bJ_build_draw[1] = 0;
    var $size_IMSU_build_draw=this.size;
    if (this.bgmode=="button") {
      ...
      if (this.bgmode=="flat") {
        if (!(this.state&UIFlags.ENABLED))
          ...
          if (this.clicked)
          canvas.box($pos_v9bJ_build_draw, $size_IMSU_build_draw, $inset_clr_Y_iP_build_draw);
        else 
          if (this.state&UIFlags.HIGHLIGHT)
          canvas.box($pos_v9bJ_build_draw, $size_IMSU_build_draw, $high_clr_JQ3J_build_draw);
    }
    if ($size_IMSU_build_draw[0]>isize[0])
      $pos_v9bJ_build_draw[0]+=($size_IMSU_build_draw[0]-isize[0])*0.5;
    if ($size_IMSU_build_draw[1]>isize[1])
      $pos_v9bJ_build_draw[1]+=($size_IMSU_build_draw[1]-isize[1])*0.5;
    if (this.small_icon)
      canvas.icon(this.icon, $pos_v9bJ_build_draw, 0.75, true);
    else 
      canvas.icon(this.icon, $pos_v9bJ_build_draw, 0.75, false);
    canvas.end(this);
  }]);
  var $pos_v9bJ_build_draw=[0, 0];
  var $high_clr_JQ3J_build_draw=[0.9, 0.9, 0.9, 0.2];
  var $size_IMSU_build_draw=[0, 0];
  var $inset_clr_Y_iP_build_draw=[0.3, 0.3, 0.3, 0.2];
