...
export class Matrix4UI extends Matrix4 {
  constructor(loc: Matrix4 | Coord, rot?: Coord, size?: Coord) {
    ...
    if (rot == undefined) rot = [0, 0, 0];

    if (size == undefined) size = [1.0, 1.0, 1.0];
...
export function simple_tri_aabb_isect(
  ...
  for (var i = 0; i < 3; i++) {
    ...
    for (var j = 0; j < 3; j++) {
      if ((vs[j][i] ?? 0) < (min[i] ?? 0) || (vs[j][i] ?? 0) >= (max[i] ?? 0)) isect = false;
    }
...
const _aabb_isect_line_2d_smin = new Vector2(),
  _aabb_isect_line_2d_smax = new Vector2();
...
const _aabb_isect_line_2d_l1: Vector2[] = [],
  _aabb_isect_line_2d_l2: Vector2[] = [];
...
export function aabb_isect_line_2d(v1: Vector2, v2: Vector2, min: Vector2, max: Vector2) {
  const smin = _aabb_isect_line_2d_smin,
    smax = _aabb_isect_line_2d_smax;
  ...
  if (!aabb_isect_2d(smin, smax, min, ssize)) return false;
  ...
  const l1 = _aabb_isect_line_2d_l1,
    l2 = _aabb_isect_line_2d_l2;
  l1[0] = v1;
  l1[1] = v2;
...
export function aabb_isect_minmax2d(
  _min1: Coord,
  _max1: Coord,
  _min2: Coord,
  _max2: Coord,
  ...
  for (var i = 0; i < 2; i++) {
    var min1 = (_min1[i] ?? 0) - margin,
      max1 = (_max1[i] ?? 0) + margin;
    var min2 = (_min2[i] ?? 0) - margin,
      max2 = (_max2[i] ?? 0) + margin;

    if (max1 >= min2 && min1 <= max2) ret += 1;
  }

  return ret == 2;
}

export function aabb_isect_2d(pos1: Coord, size1: Coord, pos2: Coord, size2: Coord) {
  var ret = 0;
...
function colinear(a: Vector3, b: Vector3, c: Vector3) {
  ...
  if (_cross_vec1.dot(_cross_vec1) < limit || _cross_vec2.dot(_cross_vec2) < limit) return true;
...
export function line_line_cross(l1: Coord[], l2: Coord[]) {
  ...
  if (Math.abs(vdist(l1[0], l2[0]) + vdist(l1[1], l2[0]) - vdist(l1[0], l1[1])) < limit) {
    return true;
  }
  if (Math.abs(vdist(l1[0], l2[1]) + vdist(l1[1], l2[1]) - vdist(l1[0], l1[1])) < limit) {
    return true;
  }
  if (Math.abs(vdist(l2[0], l1[0]) + vdist(l2[1], l1[0]) - vdist(l2[0], l2[1])) < limit) {
    return true;
  }
  if (Math.abs(vdist(l2[0], l1[1]) + vdist(l2[1], l1[1]) - vdist(l2[0], l2[1])) < limit) {
    return true;
  }
  // */
  //feps*100
...
const _normal_tri_e1 = new Vector3(),
  _normal_tri_e2 = new Vector3();
...
function get_boundary_winding(points: Vector3[]) {
  ...
  if (points.length == 0) return false; /*if no points, just return an arbitrary winding*/
...
export class PlaneOps {
  ...
  convex_quad(v1: Coord3, v2: Coord3, v3: Coord3, v4: Coord3) {
    ...
    return convex_quad(
      new Vector3([v1[ax[0]], v1[ax[1]], v1[ax[2]]]),
  ...
  line_isect(v1: Coord3, v2: Coord3, v3: Coord3, v4: Coord3) {
    var ax = this.axis;
    var orig1 = new Vector3(v1),
      orig2 = new Vector3(v2);
  ...
  line_line_cross(l1: Coord3[], l2: Coord3[]) {
    ...
    return line_line_cross(
      [new Vector3([v1[ax[0]], v1[ax[1]], 0.0]), new Vector3([v2[ax[0]], v2[ax[1]], 0.0])],
      [new Vector3([v3[ax[0]], v3[ax[1]], 0.0]), new Vector3([v4[ax[0]], v4[ax[1]], 0.0])]
    );
  }
  ...
  winding(v1: Coord3, v2: Coord3, v3: Coord3) {
    ...
    return winding(
      new Vector3([v1[ax[0]], v1[ax[1]], 0.0]),
