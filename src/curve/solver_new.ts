...
const TAN_C = (
  ...
  let ta = seg1.derivative(s1, order),
    tb = seg2.derivative(s2, order);
  if (doflip < 0.0) tb.negate();
...
const HARD_TAN_C = (seg1: SplineSegment, s1: number, goal: Vector3, order: number) => {
...
export function solve(
  ...
  for (let i = 0; i < spline.handles.length; i++) {
    let h = spline.handles[i],
      seg1 = h.owning_segment,
      v = h.owning_vertex!;

    if (do_inc && !(v.flag & SplineFlags.UPDATE)) continue;
    if (!(h.flag & SplineFlags.USE_HANDLES) && v.segments.length <= 2) continue;

    if (h.hpair !== undefined && h.flag & SplineFlags.AUTO_PAIRED_HANDLE) {
      let seg2 = h.hpair.owning_segment;

      let s1 = v === seg1.v1 ? eps : 1.0 - eps,
