...
export class KeyCurrentFrame extends ToolOp {
  ...
  exec(ctx) {
    for (var v in ctx.frameset.spline.verts.selected.editable) {
...
export class CopyPoseOp extends SplineLocalToolOp {
  ...
  exec(ctx) {
    var lists = [
      ...
      ctx.spline.handles.selected.editable
    ]
...
export class DeleteFaceOp extends SplineLocalToolOp {
  ...
  exec(ctx) {
    ...
    for (var f in spline.faces.selected.editable) {
      fset.add(f);
    }
...
export class ChangeFaceZ extends SplineLocalToolOp {
  ...
  exec(ctx) {
    ...
    if (selmode & SplineTypes.SEGMENT) {
      for (var s in spline.segments.selected.editable) {
...
export class DissolveVertOp extends SplineLocalToolOp {
  ...
  exec(ctx) {
    ...
    if (this.inputs.use_verts.data) {
      ...
      for (var eid in this.inputs.verts.data) {
...
class ConnectHandlesOp extends ToolOp {
  ...
  exec(ctx) {
    ...
    for (var h in spline.handles.selected.editable) {
      if (h1 == undefined)
        h1 = h;
...
export class ShiftTimeOp extends ToolOp {
  ...
  get_curframe_animverts(ctx) {
    ...
    for (var v in pathspline.verts.selected.editable) {
      vset.add(v);
    }
...
export class SplineMirrorOp extends SplineLocalToolOp {
    ...
    exec(ctx) {
      ...
      for (var i=0; i<2; i++) {
        ...
        for (var v in list.selected.editable) {
