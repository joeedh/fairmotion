...
export function mixinGraphNode(
  parent: Function,
  ...
  class GraphNode {
    ...
    constructor(flag = 0) {
      ...
      let getflag = () => {
        let inherit =
          typeof def.flag === "object" && def.flag !== null && def.flag instanceof InheritFlag;
    ...
    /**
     ...
     wrap inputs and/or outputs in GraphNode.inherit, see example
     ...
     static nodedef() {return {
      ...
      inputs : GraphNode.inherit({
        input1 : new FloatSocket()
      }), //can inherit from parent class by wrapping in GraphNode.inherit({})
    ...
    static nodedef(): GraphNodeDef {
      return {
        ...
        uiname : "uiname",
        flag   : 0,
        inputs : {}, //can inherit from parent class by wrapping in GraphNode.inherit({})
        outputs: {},
      };
    }
    ...
    update() {
      this.graphUpdate();
      console.warn(
        "deprecated call to graph.GraphNode.prototype.update(); use graphUpdate instead"
      );
...
export const GraphNode = mixinGraphNode(Object);
...
ProxyNode.STRUCT =
  STRUCT.inherit(ProxyNode, GraphNode, "graph.ProxyNode") +
  `
  className : string; 
}
`;
...
CallbackNode.STRUCT =
  structInherit(CallbackNode, Node, "graph.CallbackNode") +
  `
}
`;
...
export class Graph {
  ...
  remove(node: GraphNodeType) {
    if (node.graph_id === -1) {
      console.warn(
        "Warning, twiced to remove node not in graph (double remove?)",
        node.graph_id,
        node
      );
      return;
      ...
      for (let k in socks2) {
        ...
        if (s1.constructor !== s2.constructor) {
          try {
            ...
            console.warn(
              "Failed to load data from old file " +
                s2.constructor.name +
                " to " +
                s1.constructor.name
            );
...
//save nodes as abstract(Object) so mixin system works
Graph.STRUCT = `
graph.Graph {
  graph_idgen : IDGen; 
