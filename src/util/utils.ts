...
if (Array.prototype.set === undefined) {
  Array.prototype.set = function set1(
    this: unknown[],
...
class GArray<T> extends Array<T> {
  ...
  remove(item: T, ignore_existence?: boolean): this {
    //ignore_existence defaults to false
    var idx = this.indexOf(item);

    if (ignore_existence == undefined) ignore_existence = false;
...
if (!window.TYPE_LOGGING_ENABLED) {
  ...
  Reflect.set(Array.prototype, Symbol.iterator, function <
    T,
...
class hashtable<K extends Keyable, V> {
  ...
  add(key: K, item: V) {
    if (!this.items.hasOwnProperty(key[Symbol.keystr]())) this.length++;
  ...
  has(item: K | KeyStr): boolean {
    if (item === undefined) console.trace();
...
interface MeshLoop extends Keyable {
  ...
  f: MeshFace;
  next: MeshLoop;
  radial_next: MeshLoop;
}
interface MeshFace extends Keyable {
  eid: number;
  _gindex: number;
  verts: Iterable<MeshVert>;
  edges: Iterable<MeshEdge>;
...
function other_tri_vert(e: MeshEdge, f: MeshFace) {
  for (var v of f.verts) {
    if (v != e.v1 && v != e.v2) return v;
  }

  return null;
}

var _sran_tab: number[] = [
  0.42858355099189227, 0.5574386030715371, 0.9436109711290556, 0.11901816474442506,
  0.05494319267999703, 0.4089598843412747, 0.9617377622975879, 0.6144736752713642,
  0.4779527665160106, 0.5358937375859902, 0.6392009453796094, 0.24893232630444684,
  0.33278166078571036, 0.23623349009987882, 0.6007015401310062, 0.3705022651967115,
  0.0225052050200355, 0.35908220770197297, 0.6762962413645864, 0.7286584766550781,
  0.19885076794257972, 0.6066651236611478, 0.23594878250486895, 0.9559806203614414,
  0.37878311003873877, 0.14489505173573436, 0.6853451367228348, 0.778201767931336,
  0.9629591508405009, 0.10159174495809686, 0.9956652458055149, 0.27241630290235785,
  0.4657146086929548, 0.7459995799823305, 0.30955785437169314, 0.7594519036966647,
  0.9003876360971134, 0.14415784566467216, 0.13837285006138467, 0.5708662986155526,
  0.04911823375362412, 0.5182157396751097, 0.24535476698939818, 0.4755762294863617,
  0.6241760808125321, 0.05480018253112229, 0.8345698022607818, 0.26287656274013016,
  0.1025239144443526,
];
...
function get_spiral(size: number) {
  if (__v3d_g_s.length == size * size) return __v3d_g_s;
  ...
  while (c < size * size - 1) {
    ...
    for (var j = 0; j < i; j++) {
      if (i % 2 == 0) {
        if (x + sign < 0 || x + sign >= size) break;
        x += sign;
      } else {
        if (y + sign < 0 || y + sign >= size) break;
        y += sign;
      }
...
function btypeof(obj: unknown) {
  if (typeof obj == "object" && obj !== null) {
    if (obj.constructor.name in _bt_h) return _bt_h[obj.constructor.name];
    else return "object";
  } else {
    return typeof obj;
  }
