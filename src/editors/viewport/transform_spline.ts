...
export class TransSplineVert extends TransDataType {
  ...
  static calc_prop_distances(ctx: FullContext, td: TransData, data: TransDataItem[]) {
    ...
    for (var tv of data) {
      if (tv.type !== TransSplineVert) continue;
  ...
  static gen_data(ctx: FullContext, td: TransData, data: TransDataItem[]) {
    ...
    for (var i = 0; i < 2; i++) {
      for (var v of i
        ? spline.handles.selected.editable(ctx)
        : spline.verts.selected.editable(ctx)) {
        var co = new Vector2(v);

        if (i) {
          var ov = v.owning_segment.handle_vertex(v);
          if (ov !== undefined && v.hidden && ov.hidden) continue;
        } else if (v.hidden) {
          continue;
        }
    ...
    for (var si = 0; si < 2; si++) {
      ...
      for (var v of list) {
        ...
        if (!edit_all_layers && !v.in_layer(layer)) continue;
    ...
    for (var v of spline.verts.selected.editable(ctx)) {
      shash.forEachPoint(v, proprad, function (v2, dis) {
        ...
        if (!edit_all_layers && !v2.in_layer(layer)) return;

        if (
          v2.type === SplineTypes.HANDLE &&
          v2.hidden &&
          (v2.owning_vertex === undefined || v2.owning_vertex.hidden)
        )
          return;
        if (v2.type === SplineTypes.VERTEX && v2.hidden) return;
  ...
  static calc_draw_aabb(ctx: FullContext, td: TransData, minmax: MinMax) {
    ...
    for (var i = 0; i < td.data.length; i++) {
      ...
      if (dv.type == SplineTypes.HANDLE) hset[dv.eid] = 1;
    }
    ...
    function rec_walk(v: SplineVertex, depth: number) {
      ...
      for (var i = 0; i < v.segments.length; i++) {
        ...
        if (v2 != undefined && v2.flag & SplineFlags.SELECT) continue;
    ...
    for (var i = 0; i < td.data.length; i++) {
      ...
      if (v.type == SplineTypes.HANDLE) v = v.owning_vertex!;
  ...
  static aabb(
    ...
    for (var i = 0; i < item.data.segments.length; i++) {
      ...
      if (selected_only && !(item.data.flag & SplineFlags.SELECT)) continue;
