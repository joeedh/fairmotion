...
function callRuntimeCallbacks(callbacks) {
  while (callbacks.length > 0) {
    ...
    if (typeof func === "number") {if (callback.arg === undefined) {wasmTable.get(func)()} else {wasmTable.get(func)(callback.arg)}} else {
...
var asmLibraryArg = {
  "e": _clock, "c": _emscripten_memcpy_big, "a": _emscripten_resize_heap, "d": _fd_write, "b": sendMessage
};
var asm = createWasm();
...
function callMain(args) {
  ...
  try {
    ...
    if (e instanceof ExitStatus) {return} else if (e == "unwind") {return} else {
      var toLog = e;
