...
export class DuplicateTransformMacro extends ToolMacro {
  ...
  static invoke(ctx : FullContext, args) {
...
/* NOTE: this whole class reaches for names the module never imports --
   Context, toolop_menu, UIFrame, PackFlags, gen_editor_switcher, Icons and
   G -- plus a gen_delete_menu() that does not exist.  It is the dead
   predecessor of toolmodes/splinetool.ts, per the header comment. */
export class SplineEditor extends View2DEditor {
  ...
  static fromSTRUCT(reader : StructReader<SplineEditor>) {
  ...
  define_keymap() {
    ...
    */
    ...
    k.add(new HotKey("L", [], function(ctx : FullContext) {
    ...
    k.add(new HotKey("L", ["SHIFT"], function(ctx : FullContext) {
      let mpos = ctx.keymap_mpos;
      let ret = ctx.spline.q.findnearest_vert(ctx.view2d, mpos, 55, undefined, ctx.view2d.edit_all_layers);
    ...
    k.add(new HotKey("Z", [], function(ctx : FullContext) {
      ctx.view2d.only_render ^= 1;
      window.redraw_viewport();
    }, "Toggle Only Render"));

    k.add(new HotKey("W", [], function(ctx : FullContext) {
      let mpos = ctx.keymap_mpos;
