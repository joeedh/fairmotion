...
export class ShiftTimeOp2 extends ToolOp<{
  ...
  constructor() {
    ...
    /* NOTE: a local, not `this.first` -- start_modal() sets the field. */
    var first = true;
...
> extends ToolOp<InputSlots & {
  ...
  undo(ctx : FullContext) {
    ...
    for (var id in undo) {
      set_select(ctx, parseInt(id), undo[id]);
    }
  }
}
