...
export function profile_start(name) {
...
export function profile_end(name) {
  profdepth--;
...
#define packer_debug(arg)
#define packer_debug_start(arg)
#define packer_debug_end(arg)

export var _static_byte = new Uint8Array([0, 0, 0, 0, 0, 0, 0, 0]);
export var _static_view = new DataView(_static_byte.buffer);
...
/*
function _st_pack_type2(data, val, obj, thestruct, field, type) {
...
/*
  if (name == undefined) {
    if (field.type.type == T_TSTRUCT)
      name = JSON.stringify(field.type.data);
    else
...
export class STRUCT {
  ...
  static chain_fromSTRUCT(cls, reader) {
    ...
    for (var k in p) {
      if (k == "__proto__" || k == "constructor" || k == "prototype" 
          || k == "priors" || k == "__prototypeid__" || k == "__statics__") 
          continue;
      
      if (k in proto) {
        //see if we're a wrapper property
        //do copy over wrapper property *values*
  ...
  _env_call(code, obj, env) {
    ...
    profile_end(profname);
    
    return ret;
  }

  write_struct(data, obj, stt) {
    profile_start(stt.name);
