...
es6_module_define('spline_base', ["struct", "mathlib", "toolprops", "eventdag"], function _spline_base_module(_es6_module) {
  ...
  var CustomDataSet=_ESClass("CustomDataSet", Array, [function CustomDataSet() {
    ...
    while ($srcs2_zfit_interp.length<srcs.length) {
      $srcs2_zfit_interp.push(0);
    }
    $srcs2_zfit_interp.length = srcs.length;
    for (var i=0; i<this.length; i++) {
        for (var j=0; j<srcs.length; j++) {
            $srcs2_zfit_interp[j] = srcs[j][i];
        }
        this[i].interp($srcs2_zfit_interp, ws);
    }
  }, function copy(src) {
    for (var i=0; i<this.length; i++) {
        this[i].copy(src[i]);
  ...
  var $flip_out_k75___get_nextprev;
  var $ret_cache_yvOe_global_to_local;
  var $_co_60Zh_global_to_local;
  var $arr_apqs_global_to_local;
  var $_vec_ncQp_global_to_local;
  var CurveEffect=_ESClass("CurveEffect", [function CurveEffect() {
    ...
    p = p._get_nextprev(donext, $flip_out_k75___get_nextprev);
    var flip=$flip_out_k75___get_nextprev[0];
    if (p==undefined) {
        return undefined;
    }
    while (i>0) {
    ...
    var co;
    if (fixed_s!=undefined) {
        $arr_apqs_global_to_local[0] = this.evaluate(fixed_s);
        $arr_apqs_global_to_local[1] = fixed_s;
        co = $arr_apqs_global_to_local;
    }
    else {
    ...
    var n2=$_vec_ncQp_global_to_local.zero().load(p).sub(co).normalize();
    n1[2] = n2[2] = 0.0;
    ...
    var ret=$ret_cache_yvOe_global_to_local.next();
    ret[0] = s;
    ret[1] = t;
    ret[2] = a;
    return ret;
  ...
  var $flip_out_k75___get_nextprev=[0];
  var $ret_cache_yvOe_global_to_local=cachering.fromConstructor(Vector3, 64);
  var $_co_60Zh_global_to_local=new Vector3();
  var $arr_apqs_global_to_local=[0, 0];
  var $_vec_ncQp_global_to_local=new Vector3();
...
es6_module_define('spline_types', ["spline_multires", "selectmode", "eventdag", "mathlib", "spline_math", "struct", "toolprops", "toolprops_iter", "spline_base", "config"], function _spline_types_module(_es6_module) {
  "use strict";
  ...
  var SplineVertex=_ESClass("SplineVertex", SplineElement, [function SplineVertex() {
    ...
    $ret_HtkX_aabb[0].load(this);
    $ret_HtkX_aabb[1].load(this);
    return $ret_HtkX_aabb;
  }), function sethide(state) {
    if (state)
      this.flag|=SplineFlags.HIDE;
    else 
  ...
  var $ret_HtkX_aabb=[new Vector3(), new Vector3()];
  ...
  var SplineSegment=_ESClass("SplineSegment", SplineElement, [function SplineSegment(v1, v2) {
    ...
    $minmax_467H_update_aabb.reset();
    min.zero();
    max.zero();
    var co=this.evaluate(0);
    $minmax_467H_update_aabb.minmax(co);
    var ds=1.0/(steps-1);
    for (var i=0, s = 0; i<steps; i++, s+=ds) {
        var co=this.evaluate(s*0.999999999);
        $minmax_467H_update_aabb.minmax(co);
    }
    min.load($minmax_467H_update_aabb.min);
    max.load($minmax_467H_update_aabb.max);
    min[2] = max[2] = 0.0;
  }, function closest_point(p, mode, fast) {
    if (fast==undefined) {
        fast = false;
  ...
  var $minmax_467H_update_aabb=new MinMax(2);
  ...
  var SplineLoopPath=_ESClass("SplineLoopPath", [function SplineLoopPath(l, f) {
    ...
    while (1) {
      ...
      l = __ival_l.value;
      $cent_T0Qv_update_winding.add(l.v);
    }
    $cent_T0Qv_update_winding.mulScalar(1.0/this.totvert);
    var wsum=0;
    var __iter_l=__get_iter(this);
    var l;
    while (1) {
      ...
      l = __ival_l.value;
      wsum+=math.winding(l.v, l.next.v, $cent_T0Qv_update_winding) ? 1 : -1;
    }
    this.winding = wsum>=0;
  }, function asArray() {
    var l=this.l;
  ...
  var $cent_T0Qv_update_winding=new Vector3();
  ...
  var SplineFace=_ESClass("SplineFace", SplineElement, [function SplineFace() {
    ...
    while (1) {
      ...
      while (1) {
        ...
        l = __ival_l.value;
        $minmax_R1FB_update_aabb.minmax(l.v.aabb[0]);
        $minmax_R1FB_update_aabb.minmax(l.v.aabb[1]);
        $minmax_R1FB_update_aabb.minmax(l.s.aabb[0]);
        $minmax_R1FB_update_aabb.minmax(l.s.aabb[1]);
      }
    }
    this._aabb[0].load($minmax_R1FB_update_aabb.min);
    this._aabb[1].load($minmax_R1FB_update_aabb.max);
  ...
  var $minmax_R1FB_update_aabb=new MinMax(3);
...
es6_module_define('spline_query', ["spline_multires", "selectmode"], function _spline_query_module(_es6_module) {
  ...
  var PI=Math.PI, abs=Math.abs, sqrt=Math.sqrt, floor=Math.floor, ceil=Math.ceil, sin=Math.sin, cos=Math.cos, acos=Math.acos, asin=Math.asin, tan=Math.tan, atan=Math.atan, atan2=Math.atan2;
  var sqrt=Math.sqrt;
  var $_mpos_uGTR_findnearest_mres;
  var $_mpos_tc87_findnearest_vert;
  var $_v_Nlhe_findnearest_mres;
  var $_v_Pp8P_findnearest_vert;
  var SplineQuery=_ESClass("SplineQuery", [function SplineQuery(spline) {
    ...
    var spline=this.spline;
    var actlayer=spline.layerset.active;
    mpos = $_mpos_uGTR_findnearest_mres.load(mpos), mpos[2] = 0.0;
    ...
    while (1) {
      ...
      while (1) {
        ...
        $_v_Nlhe_findnearest_mres.load(mapco);
        $_v_Nlhe_findnearest_mres[2] = 0.0;
        editor.project($_v_Nlhe_findnearest_mres);
        var dis=$_v_Nlhe_findnearest_mres.vectorDistance(mpos);
    ...
    var min=1e+17;
    var ret=undefined;
    mpos = $_mpos_tc87_findnearest_vert.load(mpos), mpos[2] = 0.0;
    ...
    while (1) {
      ...
      $_v_Pp8P_findnearest_vert.load(co);
      $_v_Pp8P_findnearest_vert[2] = 0.0;
      editor.project($_v_Pp8P_findnearest_vert);
      var dis=$_v_Pp8P_findnearest_vert.vectorDistance(mpos);
      if (dis<limit&&dis<min) {
          min = dis;
          ret = v;
      }
  ...
  var $_mpos_uGTR_findnearest_mres=new Vector3();
  var $_mpos_tc87_findnearest_vert=new Vector3();
  var $_v_Nlhe_findnearest_mres=new Vector3();
  var $_v_Pp8P_findnearest_vert=new Vector3();
  _es6_module.add_class(SplineQuery);
  SplineQuery = _es6_module.add_export('SplineQuery', SplineQuery);
});
es6_module_define('spline_draw', ["view2d_editor", "config", "selectmode", "mathlib", "spline_types", "spline_draw_new", "animdata", "spline_multires", "spline_element_array", "spline_math"], function _spline_draw_module(_es6_module) {
  ...
  var $lists_wfjv_sort_layer_segments=new cachering(function() {
    return [];
  }, 2);
  function sort_layer_segments(layer, spline) {
    var list=$lists_wfjv_sort_layer_segments.next();
    list.length = 0;
    var visit={}
  ...
  var $smin_MQPd_draw_spline=new Vector3();
  var $r_vXbO_draw_spline=[[0, 0], [0, 0]];
  var $smax_7EOY_draw_spline=new Vector3();
  function draw_spline(spline, redraw_rects, g, editor, selectmode, only_render, draw_normals, alpha, draw_time_helpers, curtime, ignore_layers) {
    ...
    while (1) {
      ...
      seg = __ival_seg.value;
      $smin_MQPd_draw_spline.zero().load(seg.aabb[0]);
      $smax_7EOY_draw_spline.zero().load(seg.aabb[1]);
      var skipdraw=true;
      for (var i=0; i<redraw_rects.length; i+=4) {
          $r_vXbO_draw_spline[0][0] = redraw_rects[i], $r_vXbO_draw_spline[0][1] = redraw_rects[i+1];
          $r_vXbO_draw_spline[1][0] = redraw_rects[i+2], $r_vXbO_draw_spline[1][1] = redraw_rects[i+3];
          if (aabb_isect_minmax2d($smin_MQPd_draw_spline, $smax_7EOY_draw_spline, $r_vXbO_draw_spline[0], $r_vXbO_draw_spline[1], 2)) {
              skipdraw = false;
              break;
          }
      }
  ...
  var $margin_YiQU_redraw_element=new Vector3([15, 15, 15]);
  var $aabb_DE6C_redraw_element=[new Vector3(), new Vector3()];
  function redraw_element(e, view2d) {
    $margin_YiQU_redraw_element[0] = $margin_YiQU_redraw_element[1] = $margin_YiQU_redraw_element[2] = 15.0;
    if (view2d!=undefined)
      $margin_YiQU_redraw_element.mulScalar(1.0/view2d.zoom);
    var e_aabb=e.aabb;
    $aabb_DE6C_redraw_element[0].load(e_aabb[0]), $aabb_DE6C_redraw_element[1].load(e_aabb[1]);
    $aabb_DE6C_redraw_element[0].sub($margin_YiQU_redraw_element), $aabb_DE6C_redraw_element[1].add($margin_YiQU_redraw_element);
    window.redraw_viewport($aabb_DE6C_redraw_element[0], $aabb_DE6C_redraw_element[1]);
  }
  redraw_element = _es6_module.add_export('redraw_element', redraw_element);
});
es6_module_define('spline', ["view2d_editor", "spline_math", "solver_new", "config", "struct", "eventdag", "solver", "toolops_api", "const", "spline_element_array", "lib_api", "spline_types", "spline_query", "spline_draw", "spline_multires", "selectmode"], function _spline_module(_es6_module) {
  "use strict";
  var PI=Math.PI, abs=Math.abs, sqrt=Math.sqrt, floor=Math.floor, ceil=Math.ceil, sin=Math.sin, cos=Math.cos, acos=Math.acos, asin=Math.asin, tan=Math.tan, atan=Math.atan, atan2=Math.atan2;
  ...
  var RecalcFlags=es6_import_item(_es6_module, 'spline_types', 'RecalcFlags');
  var $debug_id_gen_0ois_Spline;
  var $_internal_idgen_q_2T_Spline;
  var $ws_bw7Z_split_edge;
  var $lastco_MNMa_trace_face;
  var $srcs_oBL3_split_edge;
  var Spline=_ESClass("Spline", DataBlock, [function Spline(name) {
    if (name==undefined) {
        name = undefined;
    }
    DataBlock.call(this, DataTypes.SPLINE, name);
    this._debug_id = $debug_id_gen_0ois_Spline++;
    ...
    this._internal_id = $_internal_idgen_q_2T_Spline++;
    this.drawlist = [];
    ...
    $srcs_oBL3_split_edge[0] = v1.cdata, $srcs_oBL3_split_edge[1] = v2.cdata;
    this.copy_vert_data(nv, v1);
    nv.cdata.interp($srcs_oBL3_split_edge, $ws_bw7Z_split_edge);
    this.resolve = 1;
    return ret;
  ...
  var $debug_id_gen_0ois_Spline=0;
  var $_internal_idgen_q_2T_Spline=0;
  var $ws_bw7Z_split_edge=[0.5, 0.5];
  var $lastco_MNMa_trace_face=new Vector3();
  var $srcs_oBL3_split_edge=[0, 0];
...
es6_module_define('spline_multires', ["binomial_table", "struct", "spline_base"], function _spline_multires_module(_es6_module) {
  ...
  var BoundPoint=_ESClass("BoundPoint", [function BoundPoint() {
    ...
    $p_B9OO_recalc_offset[0] = this[0];
    $p_B9OO_recalc_offset[1] = this[1];
    var sta=seg._evalwrap.global_to_local($p_B9OO_recalc_offset, undefined, this.s);
    this.t = sta[1];
    this.a = sta[2];
  }, function toString() {
  ...
  var $p_B9OO_recalc_offset=new Vector3([0, 0, 0]);
  ...
  var $sum_bBnY_evaluate;
  var $ks_g7A9_evaluate;
  var MultiResEffector=_ESClass("MultiResEffector", CurveEffect, [function MultiResEffector(owner) {
    ...
    $sum_bBnY_evaluate.zero();
    var i=0;
    for (var p in this.mr.points(0)) {
        $ks_g7A9_evaluate[i] = p.s;
        i++;
    }
    for (var p in this.mr.points(0)) {
        ...
        if (isNaN(w))
          continue;
        $sum_bBnY_evaluate[0]+=p.offset[0]*w;
        $sum_bBnY_evaluate[1]+=p.offset[1]*w;
    }
    for (var i=0; i<2; i++) {
    ...
    co.add($sum_bBnY_evaluate);
    return co;
  }]);
  var $sum_bBnY_evaluate=new Vector3();
  var $ks_g7A9_evaluate=new Array(2000);
  ...
  var $_co_UTTD_add_point;
  var $sta_jKLh_recalc_worldcos_level;
  var MultiResLayer=_ESClass("MultiResLayer", CustomDataLayer, [function MultiResLayer(size) {
    ...
    for (var p in this.points(level)) {
        $sta_jKLh_recalc_worldcos_level[0] = p.s;
        $sta_jKLh_recalc_worldcos_level[1] = p.t;
        $sta_jKLh_recalc_worldcos_level[2] = p.a;
        var co=seg._evalwrap.local_to_global($sta_jKLh_recalc_worldcos_level);
        var co2=seg._evalwrap.evaluate($sta_jKLh_recalc_worldcos_level[0]);
        p[0] = co[0];
        p[1] = co[1];
  ...
  var $_co_UTTD_add_point=[0, 0];
  var $sta_jKLh_recalc_worldcos_level=[0, 0, 0];
  ...
  var $ret_xP5j_decompose_id=[0, 0];
  function decompose_id(id) {
    var mul=(1<<24);
    var eid=Math.floor(id/mul);
    id-=eid*mul;
    $ret_xP5j_decompose_id[0] = eid;
    $ret_xP5j_decompose_id[1] = id;
    return $ret_xP5j_decompose_id;
  }
...
es6_module_define('solver_new', ["spline_base", "spline_math"], function _solver_new_module(_es6_module) {
  ...
  var $tan_7YJn_solve=new Vector3();
        ...
        while (i<plen&&i>=0) {
          ...
          for (var ci=0; ci<2; ci++) {
              ...
              if (seg2!=undefined) {
                ...
                var h=seg1.handle(v);
                $tan_7YJn_solve.load(h).sub(v).normalize();
                if (v==seg1.v2)
                  $tan_7YJn_solve.negate();
                var ta=seg1.derivative(s1, order).normalize();
                var _d=Math.min(Math.max(ta.dot($tan_7YJn_solve), -1.0), 1.0);
                var r=acos(_d);
...
es6_module_define('vectordraw_canvas2d_simple', ["config", "mathlib", "vectordraw_base"], function _vectordraw_canvas2d_simple_module(_es6_module) {
  "use strict";
  var config=es6_import(_es6_module, 'config');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var VectorFlags=es6_import_item(_es6_module, 'vectordraw_base', 'VectorFlags');
...
es6_module_define('vectordraw', ["vectordraw_canvas2d", "vectordraw_base", "vectordraw_svg"], function _vectordraw_module(_es6_module) {
  "use strict";
...
es6_module_define('spline_draw_new', ["config", "spline_multires", "mathlib", "animdata", "vectordraw", "spline_math", "view2d_editor", "spline_element_array", "spline_types", "selectmode"], function _spline_draw_new_module(_es6_module) {
  "use strict";
  var aabb_isect_minmax2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_minmax2d');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var ENABLE_MULTIRES=es6_import_item(_es6_module, 'config', 'ENABLE_MULTIRES');
...
es6_module_define('events', [], function _events_module(_es6_module) {
  ...
  var $vel_vFBQ_on_tick;
  var $vel__cPX_calc_vel;
  var $was_clamped_QsCu_clamp_pan;
  var VelocityPan=_ESClass("VelocityPan", EventHandler, [function VelocityPan() {
    ...
    if (!this.panning&&this.coasting) {
        var damp=0.99;
        $vel_vFBQ_on_tick.load(this.vel);
        $vel_vFBQ_on_tick.mulScalar(time_ms()-this.last_ms);
        this.vel.mulScalar(damp);
        this.last_ms = time_ms();
        this.pan.sub($vel_vFBQ_on_tick);
    ...
    $vel__cPX_calc_vel.load(this.last_mpos).sub(this.mpos).divideScalar(t);
    this.vel.add($vel__cPX_calc_vel);
    ...
    if (this.owner.state&8192*4)
      return ;
    var p=this.pan;
    $was_clamped_QsCu_clamp_pan[0] = false;
    $was_clamped_QsCu_clamp_pan[1] = false;
    for (var i=0; i<2; i++) {
        var l=p[i];
        p[i] = Math.min(Math.max(bs[0][i], p[i]), bs[0][i]+bs[1][i]);
        if (p[i]!=l)
          $was_clamped_QsCu_clamp_pan[i] = true;
    }
    return $was_clamped_QsCu_clamp_pan;
  }, function on_mouseup(event) {
  ...
  var $vel_vFBQ_on_tick=new Vector2();
  var $vel__cPX_calc_vel=new Vector2();
  var $was_clamped_QsCu_clamp_pan=[0, 0];
...
es6_module_define('toolprops', ["ajax", "struct", "toolprops_iter"], function _toolprops_module(_es6_module) {
  "use strict";
  var STRUCT=es6_import_item(_es6_module, 'struct', 'STRUCT');
...
es6_module_define('toolprops_iter', ["struct"], function _toolprops_iter_module(_es6_module) {
  ...
  var element_iter_convert=_ESClass("element_iter_convert", ToolIter, [function element_iter_convert(iter, type) {
    ...
    var ob={}
    reader(ob);
    var type=$map_dm6l_fromSTRUCT[ob.type];
    var ret=new element_iter_convert(ob._iter, type);
  })]);
  var $map_dm6l_fromSTRUCT={Vertex: 1, Edge: 2, Loop: 4, Face: 8}
...
es6_module_define('toolops_api', ["events", "struct", "toolprops"], function _toolops_api_module(_es6_module) {
  ...
  var $toolops_D2V0_get_constructor;
  var ToolOp=_ESClass("ToolOp", ToolOpAbstract, [function ToolOp(apiname, uiname, description, icon) {
    ...
    if ($toolops_D2V0_get_constructor==undefined) {
        $toolops_D2V0_get_constructor = {};
        for (var c in defined_classes) {
            if (__instance_of(c, ToolOp))
              $toolops_D2V0_get_constructor[c.name] = c;
        }
    }
    return $toolops_D2V0_get_constructor[c];
  })]);
  var $toolops_D2V0_get_constructor=undefined;
  ...
  var WidgetToolOp=_ESClass("WidgetToolOp", ToolOp, [_ESClass.static(function create_widgets(manager, ctx) {
    ...
    function widget_on_tick(widget) {
      ...
      if (this2.widget_align_normal) {
          ...
          var angle=Math.PI-Math.acos($zaxis_m6r2.dot(n));
          if (n.dot($zaxis_m6r2)>0.9) {
          }
          if (1) {
    ...
    var $zaxis_m6r2=new Vector3([0, 0, -1]);
...
es6_module_define('eventdag', ["J3DIMath"], function _eventdag_module(_es6_module) {
  ...
  var EventDag=_ESClass("EventDag", [function EventDag() {
    ...
    if (!(__instance_of(srcfield, Array))) {
        $sarr_AO3s_link[0] = srcfield;
        srcfield = $sarr_AO3s_link;
    }
    if (!(__instance_of(dstfield, Array))) {
        $darr_ducC_link[0] = dstfield;
        dstfield = $darr_ducC_link;
    }
  ...
  var $sarr_AO3s_link=[0];
  var $darr_ducC_link=[0];
