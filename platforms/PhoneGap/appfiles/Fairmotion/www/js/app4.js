es6_module_define('native_api', ["spline_math_hermite", "typedwriter", "spline_base", "ajax", "toolops_api", "built_wasm", "solver"], function _native_api_module(_es6_module) {
...
}, '/dev/cleanfairmotion/src/wasm/native_api.js');
...
}, '/dev/cleanfairmotion/src/addon_api/addon_api.js');
...
}, '/dev/cleanfairmotion/src/scene/scene.js');
es6_module_define('events', ["J3DIMath"], function _events_module(_es6_module) {
  ...
  var $vel_SltP_on_tick;
  var $vel_ZzB0_calc_vel;
  var $was_clamped_aSRd_clamp_pan;
  var VelocityPan=_ESClass("VelocityPan", EventHandler, [function VelocityPan() {
    ...
    if (!this.panning&&this.coasting) {
        var damp=0.99;
        $vel_SltP_on_tick.load(this.vel);
        $vel_SltP_on_tick.mulScalar(time_ms()-this.last_ms);
        this.vel.mulScalar(damp);
        this.last_ms = time_ms();
        this.pan.sub($vel_SltP_on_tick);
    ...
    $vel_ZzB0_calc_vel.load(this.last_mpos).sub(this.mpos).divideScalar(t);
    this.vel.add($vel_ZzB0_calc_vel);
    ...
    if (this.owner.state&8192*4)
      return ;
    var p=this.pan;
    $was_clamped_aSRd_clamp_pan[0] = false;
    $was_clamped_aSRd_clamp_pan[1] = false;
    for (var i=0; i<2; i++) {
        var l=p[i];
        p[i] = Math.min(Math.max(bs[0][i], p[i]), bs[0][i]+bs[1][i]);
        if (p[i]!=l)
          $was_clamped_aSRd_clamp_pan[i] = true;
    }
    return $was_clamped_aSRd_clamp_pan;
  }, function on_mouseup(event) {
  ...
  var $vel_SltP_on_tick=new Vector2();
  var $vel_ZzB0_calc_vel=new Vector2();
  var $was_clamped_aSRd_clamp_pan=[0, 0];
...
}, '/dev/cleanfairmotion/src/editors/viewport/events.js');
...
}, '/dev/cleanfairmotion/src/ui/touchevents.js');
...
}, '/dev/cleanfairmotion/src/core/toolprops.js');
es6_module_define('toolprops_iter', ["struct"], function _toolprops_iter_module(_es6_module) {
  ...
  var element_iter_convert=_ESClass("element_iter_convert", ToolIter, [function element_iter_convert(iter, type) {
    ...
    var ob={}
    reader(ob);
    var type=$map_7vJ6_fromSTRUCT[ob.type];
    var ret=new element_iter_convert(ob._iter, type);
  })]);
  var $map_7vJ6_fromSTRUCT={Vertex: 1, Edge: 2, Loop: 4, Face: 8}
...
}, '/dev/cleanfairmotion/src/core/toolprops_iter.js');
es6_module_define('toolops_api', ["struct", "toolprops", "../editors/viewport/events"], function _toolops_api_module(_es6_module) {
  ...
  var ToolOp=_ESClass("ToolOp", ToolOpAbstract, [function ToolOp(apiname, uiname, description, icon) {
    ...
    if (_toolop_tools===undefined) {
        _toolop_tools = {};
        var __iter_c=__get_iter(defined_classes);
        var c;
        while (1) {
  ...
  var ToolMacro=_ESClass("ToolMacro", ToolOp, [function ToolMacro(name, uiname, tools) {
    ...
    var __iter_op=__get_iter(this.tools);
    var op;
    while (1) {
      var __ival_op=__iter_op.next();
      if (__ival_op.done) {
          break;
      }
      op = __ival_op.value;
  ...
  var WidgetToolOp=_ESClass("WidgetToolOp", ToolOp, [_ESClass.static(function create_widgets(manager, ctx) {
    ...
    function widget_on_tick(widget) {
      ...
      if (this2.widget_align_normal) {
          ...
          var angle=Math.PI-Math.acos($zaxis_yYp2.dot(n));
          if (n.dot($zaxis_yYp2)>0.9) {
          }
          if (1) {
    ...
    var $zaxis_yYp2=new Vector3([0, 0, -1]);
...
}, '/dev/cleanfairmotion/src/core/toolops_api.js');
es6_module_define('eventdag', ["J3DIMath"], function _eventdag_module(_es6_module) {
  ...
  var EventDag=_ESClass("EventDag", [function EventDag() {
    ...
    if (!(__instance_of(srcfield, Array))) {
        $sarr_quwz_link[0] = srcfield;
        srcfield = $sarr_quwz_link;
    }
    if (!(__instance_of(dstfield, Array))) {
        $darr_b0wx_link[0] = dstfield;
        dstfield = $darr_b0wx_link;
    }
  ...
  var $sarr_quwz_link=[0];
  var $darr_b0wx_link=[0];
...
}, '/dev/cleanfairmotion/src/core/eventdag.js');
es6_module_define('lib_utils', ["../editors/viewport/events", "struct", "toolprops_iter"], function _lib_utils_module(_es6_module) {
  "use strict";
...
}, '/dev/cleanfairmotion/src/core/lib_utils.js');
...
}, '/dev/cleanfairmotion/src/editors/viewport/transdata.js');
es6_module_define('transform', ["transdata", "dopesheet_transdata", "./events", "toolops_api", "selectmode", "mathlib", "native_api", "spline_types", "toolprops"], function _transform_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  ...
  var TransSplineVert=_ESClass("TransSplineVert", [_ESClass.static(function apply(ctx, td, item, mat, w) {
    ...
    var __iter_v=__get_iter(spline.verts.selected.editable(ctx));
    var v;
    while (1) {
      var __ival_v=__iter_v.next();
      if (__ival_v.done) {
    ...
    var edit_all_layers=td.edit_all_layers;
    for (var i=0; i<2; i++) {
        var __iter_v=__get_iter(i ? spline.handles.selected.editable(ctx) : spline.verts.selected.editable(ctx));
        var v;
        while (1) {
          var __ival_v=__iter_v.next();
          if (__ival_v.done) {
...
}, '/dev/cleanfairmotion/src/editors/viewport/transform.js');
es6_module_define('transform_ops', ["mathlib", "./events", "multires_transdata", "selectmode", "spline_types", "native_api", "transdata", "toolprops", "transform", "dopesheet_transdata", "toolops_api"], function _transform_ops_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  ...
  var WidgetResizeOp=_ESClass("WidgetResizeOp", TransformOp, [function WidgetResizeOp(user_start_mpos, datamode) {
    ...
    if (ctx.view2d.selectmode&SelMask.HANDLE) {
        var __iter_h=__get_iter(spline.handles.selected.editable(ctx));
        var h;
        while (1) {
    ...
    var __iter_seg=__get_iter(spline.segments.selected.editable(ctx));
    var seg;
    while (1) {
...
}, '/dev/cleanfairmotion/src/editors/viewport/transform_ops.js');
es6_module_define('spline_selectops', ["spline_draw", "spline_types", "toolops_api", "toolprops", "animdata"], function _spline_selectops_module(_es6_module) {
  "use strict";
  var $_mh;
  var $_swapt;
...
}, '/dev/cleanfairmotion/src/editors/viewport/spline_selectops.js');
es6_module_define('spline_createops', ["spline_editops", "toolops_api", "spline", "spline_types", "toolprops"], function _spline_createops_module(_es6_module) {
  var ToolOp=es6_import_item(_es6_module, 'toolops_api', 'ToolOp');
  var SplineFlags=es6_import_item(_es6_module, 'spline_types', 'SplineFlags');
  var EnumProperty=es6_import_item(_es6_module, 'toolprops', 'EnumProperty');
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
...
}, '/dev/cleanfairmotion/src/editors/viewport/spline_createops.js');
es6_module_define('spline_editops', ["toolprops", "spline", "struct", "spline_types", "spline_draw", "animdata", "toolops_api", "frameset", "../../curve/spline_base"], function _spline_editops_module(_es6_module) {
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
  var FloatProperty=es6_import_item(_es6_module, 'toolprops', 'FloatProperty');
  var CollectionProperty=es6_import_item(_es6_module, 'toolprops', 'CollectionProperty');
  var BoolProperty=es6_import_item(_es6_module, 'toolprops', 'BoolProperty');
  ...
  var KeyCurrentFrame=_ESClass("KeyCurrentFrame", ToolOp, [function KeyCurrentFrame() {
    ...
    var __iter_v=__get_iter(ctx.frameset.spline.verts.selected.editable(ctx));
    var v;
    while (1) {
      var __ival_v=__iter_v.next();
      if (__ival_v.done) {
  ...
  var DeleteSegmentOp=_ESClass("DeleteSegmentOp", ToolOp, [function DeleteSegmentOp() {
    ...
    var spline=ctx.spline;
    var dellist=[];
    var __iter_s=__get_iter(spline.segments.selected.editable(ctx));
    var s;
    while (1) {
      var __ival_s=__iter_s.next();
      if (__ival_s.done) {
  ...
  var DeleteFaceOp=_ESClass("DeleteFaceOp", SplineLocalToolOp, [function DeleteFaceOp() {
    ...
    var dellist=[];
    var __iter_f=__get_iter(spline.faces.selected.editable(ctx));
    var f;
    while (1) {
      var __ival_f=__iter_f.next();
      if (__ival_f.done) {
  ...
  var SplitEdgePickOp=_ESClass("SplitEdgePickOp", SplineGlobalToolOp, [function SplitEdgePickOp() {
    ...
    return {uiname: "Split Segment", apiname: "spline.split_pick_edge", inputs: {segment_eid: new IntProperty(-1, "segment_eid", "segment_eid", "segment_eid"), segment_t: new FloatProperty(0, "segment_t", "segment_t", "segment_t"), spline_path: new StringProperty("drawspline", "spline_path", "splien_path", "spline_path"), deselect: new BoolProperty(true, "deselect", "deselect", "deselect")}, outputs: {}, icon: Icons.SPLIT_EDGE, is_modal: true, description: "Split picked segment"}
  }), function can_call(ctx) {
    return !(ctx.spline.restrict&RestrictFlags.NO_SPLIT_EDGE);
  }, function start_modal(ctx) {
  ...
  var ToggleBreakTanOp=_ESClass("ToggleBreakTanOp", VertPropertyBaseOp, [function ToggleBreakTanOp() {
    ...
    for (var si=0; si<2; si++) {
        var list=si ? spline.handles : spline.verts;
        var __iter_v=__get_iter(list.selected.editable(ctx));
        var v;
        while (1) {
          var __ival_v=__iter_v.next();
          if (__ival_v.done) {
  ...
  var ShiftTimeOp=_ESClass("ShiftTimeOp", ToolOp, [function ShiftTimeOp() {
    ...
    var spline=ctx.frameset.spline, pathspline=ctx.frameset.pathspline;
    var frameset=ctx.frameset;
    var __iter_v=__get_iter(pathspline.verts.selected.editable(ctx));
    var v;
    while (1) {
      var __ival_v=__iter_v.next();
      if (__ival_v.done) {
...
}, '/dev/cleanfairmotion/src/editors/viewport/spline_editops.js');
es6_module_define('spline_layerops', ["toolprops", "toolops_api", "spline", "spline_types", "spline_editops"], function _spline_layerops_module(_es6_module) {
  var ToolOp=es6_import_item(_es6_module, 'toolops_api', 'ToolOp');
  var UndoFlags=es6_import_item(_es6_module, 'toolops_api', 'UndoFlags');
  var ToolFlags=es6_import_item(_es6_module, 'toolops_api', 'ToolFlags');
  var SplineFlags=es6_import_item(_es6_module, 'spline_types', 'SplineFlags');
...
}, '/dev/cleanfairmotion/src/editors/viewport/spline_layerops.js');
es6_module_define('spline_animops', [], function _spline_animops_module(_es6_module) {
}, '/dev/cleanfairmotion/src/editors/viewport/spline_animops.js');
es6_module_define('multires_ops', ["spline", "spline_editops", "toolprops", "spline_multires", "spline_draw", "toolops_api", "J3DIMath", "spline_types"], function _multires_ops_module(_es6_module) {
  es6_import(_es6_module, 'J3DIMath');
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
  var FloatProperty=es6_import_item(_es6_module, 'toolprops', 'FloatProperty');
  var CollectionProperty=es6_import_item(_es6_module, 'toolprops', 'CollectionProperty');
  ...
  var CreateMResPoint=_ESClass("CreateMResPoint", SplineLocalToolOp, [function CreateMResPoint(seg, co) {
    ...
    if (cp!=undefined) {
        ...
        $vec_4v0T_exec.zero().load(co).sub(cp[0]);
        var n=seg.normal(s);
        t*=Math.sign(n.dot($vec_4v0T_exec));
        p.offset[0] = $vec_4v0T_exec[0];
        p.offset[1] = $vec_4v0T_exec[1];
    }
    else {
...
}, '/dev/cleanfairmotion/src/editors/viewport/multires/multires_ops.js');
es6_module_define('multires_selectops', ["toolprops", "spline_editops", "spline_types", "toolops_api", "spline", "spline_draw", "J3DIMath", "spline_multires"], function _multires_selectops_module(_es6_module) {
  "use strict";
  es6_import(_es6_module, 'J3DIMath');
  var IntProperty=es6_import_item(_es6_module, 'toolprops', 'IntProperty');
  var FloatProperty=es6_import_item(_es6_module, 'toolprops', 'FloatProperty');
...
}, '/dev/cleanfairmotion/src/editors/viewport/multires/multires_selectops.js');
es6_module_define('multires_transdata', ["mathlib", "transdata", "selectmode", "spline_multires"], function _multires_transdata_module(_es6_module) {
  "use strict";
  var SelMask=es6_import_item(_es6_module, 'selectmode', 'SelMask');
  var compose_id=es6_import_item(_es6_module, 'spline_multires', 'compose_id');
  var decompose_id=es6_import_item(_es6_module, 'spline_multires', 'decompose_id');
  ...
  var TransDataItem=es6_import_item(_es6_module, 'transdata', 'TransDataItem');
  var $co_5y51_apply;
  var $co_plBb_calc_draw_aabb;
  var $co2_8YT__calc_draw_aabb;
  var $co_Qmkj_aabb;
  var MResTransData=_ESClass("MResTransData", TransDataType, [_ESClass.static(function gen_data(ctx, td, data) {
    ...
    if (w==0.0)
      return ;
    $co_5y51_apply.load(item.start_data);
    $co_5y51_apply[2] = 0.0;
    $co_5y51_apply.multVecMatrix(mat);
    $co_5y51_apply.sub(item.start_data).mulScalar(w).add(item.start_data);
    p[0] = $co_5y51_apply[0];
    p[1] = $co_5y51_apply[1];
    ...
    $co_plBb_calc_draw_aabb.zero();
    var pad=15;
    function do_minmax(co) {
      $co2_8YT__calc_draw_aabb[0] = co[0]-pad;
      $co2_8YT__calc_draw_aabb[1] = co[1]-pad;
      minmax.minmax($co2_8YT__calc_draw_aabb);
      $co2_8YT__calc_draw_aabb[0]+=pad*2.0;
      $co2_8YT__calc_draw_aabb[1]+=pad*2.0;
      minmax.minmax($co2_8YT__calc_draw_aabb);
    }
    var spline=ctx.spline;
    for (var i=0; i<td.data.length; i++) {
        ...
        $co_plBb_calc_draw_aabb[0] = t.data[0];
        $co_plBb_calc_draw_aabb[1] = t.data[1];
        do_minmax($co_plBb_calc_draw_aabb);
        $co_plBb_calc_draw_aabb[0]-=t.data.offset[0];
        $co_plBb_calc_draw_aabb[1]-=t.data.offset[1];
        do_minmax($co_plBb_calc_draw_aabb);
    }
  }), _ESClass.static(function aabb(ctx, td, item, minmax, selected_only) {
    $co_Qmkj_aabb.zero();
    for (var i=0; i<td.data.length; i++) {
        ...
        if (t.type!==MResTransData)
          continue;
        $co_Qmkj_aabb[0] = t.data[0];
        $co_Qmkj_aabb[1] = t.data[1];
        minmax.minmax($co_Qmkj_aabb);
    }
  }), function MResTransData() {
    TransDataType.apply(this, arguments);
  }]);
  var $co_5y51_apply=new Vector3();
  var $co_plBb_calc_draw_aabb=new Vector3();
  var $co2_8YT__calc_draw_aabb=[0, 0, 0];
  var $co_Qmkj_aabb=new Vector3();
...
}, '/dev/cleanfairmotion/src/editors/viewport/multires/multires_transdata.js');
var g_theme;
...
}, '/dev/cleanfairmotion/src/ui/theme.js');
...
}, '/dev/cleanfairmotion/src/ui/theme_def.js');
es6_module_define('UIElement', ["mathlib", "toolprops", "../editors/viewport/events"], function _UIElement_module(_es6_module) {
  es6_import(_es6_module, '../editors/viewport/events');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  ...
  var $empty_arr_jxok_get_keymaps;
  var $pos_SqRc_get_abs_pos;
  var $ret_i9Xu_get_min_size;
  var UIElement=_ESClass("UIElement", EventHandler, [function UIElement(ctx, path, pos, size) {
    ...
    return $empty_arr_jxok_get_keymaps;
  }, _ESClass.symbol(Symbol.keystr, function keystr() {
    ...
    $pos_SqRc_get_abs_pos[0] = this.pos[0];
    $pos_SqRc_get_abs_pos[1] = this.pos[1];
    var p=this.parent;
    while (p!=undefined) {
      $pos_SqRc_get_abs_pos[0]+=p.pos[0];
      $pos_SqRc_get_abs_pos[1]+=p.pos[1];
      p = p.parent;
    }
    return $pos_SqRc_get_abs_pos;
  ...
  var $empty_arr_jxok_get_keymaps=[];
  var $pos_SqRc_get_abs_pos=[0, 0];
  var $ret_i9Xu_get_min_size=[1, 1];
...
}, '/dev/cleanfairmotion/src/ui/UIElement.js');
...
}, '/dev/cleanfairmotion/src/ui/UIFileData.js');
es6_module_define('UICanvas', ["UIElement", "mathlib"], function _UICanvas_module(_es6_module) {
  ...
  var $ret_Sk3V_get_2d_canvas={}
  function get_2d_canvas() {
    if ($ret_Sk3V_get_2d_canvas.canvas==undefined) {
        $ret_Sk3V_get_2d_canvas.canvas = document.getElementById("canvas2d");
        $ret_Sk3V_get_2d_canvas.ctx = _canvas2d_ctx;
    }
    return $ret_Sk3V_get_2d_canvas;
  }
  get_2d_canvas = _es6_module.add_export('get_2d_canvas', get_2d_canvas);
  window.get_2d_canvas = get_2d_canvas;
  var $ret_xxXw_get_2d_canvas_2={}
  function get_2d_canvas_2() {
    if ($ret_xxXw_get_2d_canvas_2.canvas==undefined) {
        $ret_xxXw_get_2d_canvas_2.canvas = document.getElementById("canvas2d_work");
        $ret_xxXw_get_2d_canvas_2.ctx = _canvas2d_ctx_2;
    }
    return $ret_xxXw_get_2d_canvas_2;
  }
  ...
  var $temp_layer_idgen_25ni_push_layer;
  var $black_J1cw_quad;
  var $grads_beQx_quad;
  var $mid_uA7E_colorfield;
  var $cache__UAA_box1;
  var $v1_Q1CS_box1;
  var $v3_tQWU_box1;
  var $pairs_opv8_box1;
  ...
  var UICanvas=_ESClass("UICanvas", [function UICanvas(viewport) {
    ...
    var canvas=document.createElement("canvas");
    canvas.id = "_temp_canvas2d_"+($temp_layer_idgen_25ni_push_layer++);
    document.body.appendChild(canvas);
    canvas.style["position"] = "absolute";
    canvas.style["left"] = "0px";
    canvas.style["top"] = "0px";
    ...
    $mid_uA7E_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        if (color[i]==0.0)
          $mid_uA7E_colorfield[i] = 0.0;
        else 
          $mid_uA7E_colorfield[i] = color[i];
    }
    var color2=this._css_color($mid_uA7E_colorfield);
    $mid_uA7E_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        $mid_uA7E_colorfield[i] = (color[i]*3.0-1.0)/4.0;
    }
    var midclr=this._css_color($mid_uA7E_colorfield);
    $mid_uA7E_colorfield[3] = 1.0;
    for (var i=0; i<3; i++) {
        $mid_uA7E_colorfield[i] = 0.5+color[i]*0.5;
    }
    var smidclr=this._css_color($mid_uA7E_colorfield);
    $mid_uA7E_colorfield[3] = 0.0;
    for (var i=0; i<3; i++) {
        $mid_uA7E_colorfield[i] = color[i];
    }
    var zerocolor=this._css_color($mid_uA7E_colorfield);
    ...
    if (!(hash in $cache__UAA_box1)) {
        ...
        p2.reverse();
        $cache__UAA_box1[hash] = [p1, p2, points];
    }
    var cp=$cache__UAA_box1[hash];
    var p1=cp[0];
    var p2=cp[1];
    var points=cp[2];
    var plen=p1.length;
    ...
    if (!outline_only) {
        for (var i=0; i<p1.length-1; i++) {
            ...
            var i3=i+1+plen*2;
            var i4=i+1;
            $v1_Q1CS_box1[0] = p1[i][0]+x;
            $v1_Q1CS_box1[1] = p1[i][1]+y;
            $v1_Q1CS_box1[2] = p1[i][2];
            
            $v2_7AsI_box1[0] = p2[i][0]+x;
            $v2_7AsI_box1[1] = p2[i][1]+y;
            $v2_7AsI_box1[2] = p2[i][2];
            
            $v3_tQWU_box1[0] = p2[i+1][0]+x;
            $v3_tQWU_box1[1] = p2[i+1][1]+y;
            $v3_tQWU_box1[2] = p2[i+1][2];
            
            $v4_R7qx_box1[0] = p1[i+1][0]+x;
            $v4_R7qx_box1[1] = p1[i+1][1]+y;
            $v4_R7qx_box1[2] = p1[i+1][2];
            
            this.quad($v1_Q1CS_box1, $v2_7AsI_box1, $v3_tQWU_box1, $v4_R7qx_box1, color(i1), color(i2), color(i3), color(i4));
        }
    }
    var lines=[];
    var colors=[];
    for (var i=0; i<points.length; i++) {
        $v1_Q1CS_box1[0] = points[(i+1)%points.length][0]+x;
        $v1_Q1CS_box1[1] = points[(i+1)%points.length][1]+y;
        $v1_Q1CS_box1[2] = points[(i+1)%points.length][2];
        
        $v2_7AsI_box1[0] = points[i][0]+x;
        $v2_7AsI_box1[1] = points[i][1]+y;
        $v2_7AsI_box1[2] = points[i][2];
        
        if ($pairs_opv8_box1.length<=i) {
            $pairs_opv8_box1.push([[0, 0], [0, 0]]);
        }
        $pairs_opv8_box1[i][0][0] = (($_mh = objcache.array(2)), ($_mh[0] = ($v1_Q1CS_box1[0])), ($_mh[1] = ($v1_Q1CS_box1[1])), ($_mh[2] = (0)), $_mh);
        $pairs_opv8_box1[i][0][1] = (($_mh = objcache.array(2)), ($_mh[0] = ($v2_7AsI_box1[0])), ($_mh[1] = ($v2_7AsI_box1[1])), ($_mh[2] = (0)), $_mh);
        lines.push($pairs_opv8_box1[i][0]);
        $pairs_opv8_box1[i][1][0] = color((i+1)%points.length);
        $pairs_opv8_box1[i][1][1] = color(i);
        colors.push($pairs_opv8_box1[i][1]);
    }
    ...
    for (var i=0; i<lines.length; i++, ly+=12) {
        ...
        var m=this.transmat.$matrix;
        $pos_jCg7_text[0] = m.m41+v[0][0]+pos1[0];
        $pos_jCg7_text[1] = canvas.height-(m.m42+v[0][1]+pos1[1]+ly);
        $pos_jCg7_text[2] = 0;
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.rotate(rot);
        if (rot!=0) {
        ...
        rot2d($pos_jCg7_text, -rot);
        $pos_jCg7_text[1] = canvas.height-$pos_jCg7_text[1];
        if (color==undefined)
        ...
        var x=$pos_jCg7_text[0], y=canvas.height-($pos_jCg7_text[1]);
  ...
  var $temp_layer_idgen_25ni_push_layer=0;
  var $black_J1cw_quad=[0, 0, 0, 1];
  var $grads_beQx_quad={}
  var $mid_uA7E_colorfield=[0, 0, 0, 0.5];
  var $cache__UAA_box1={}
  var $v1_Q1CS_box1=new Vector3();
  var $v3_tQWU_box1=new Vector3();
  var $pairs_opv8_box1=[];
  var $pos_jCg7_text=[0, 0, 0];
  var $v2_7AsI_box1=new Vector3();
  var $v4_R7qx_box1=new Vector3();
...
}, '/dev/cleanfairmotion/src/ui/UICanvas.js');
