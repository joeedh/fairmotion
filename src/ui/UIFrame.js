...
export class UIFrame extends UIElement {
  ...
  do_full_recalc()
  {
    //console.trace("basic full recalc");
    
    this.dirty_rects.push([
      [this.abspos[0], this.abspos[1]],
  ...
  focus(UIElement e) {
    if (this.active !== e && this.active !== undefined) {
      ...
      this.active.on_inactive();
      this.active.do_recalc();
    } else if (e != undefined && e === this.active) {
      if (this.parent != undefined)
...
//        pos[1] += c.velpan.pan[1];
  ...
  build_draw_old(canvas, isVertical, cache_frame=undefined) {
    var mat = _static_mat;
    this.has_hidden_elements = false;
    ...
    for (var c of this.children) {
      ...
      if (!(c instanceof UIFrame) && this.constructor.name != "UIMenu") {
