es6_module_define('UIFrame', ["../editors/viewport/events", "J3DIMath", "UIElement", "mathlib", "config"], function _UIFrame_module(_es6_module) {
  ...
  var $pos_KJX8__find_active;
  var $zero_hrME_build_draw_old;
  var UIFrame=_ESClass("UIFrame", UIElement, [function UIFrame(ctx, canvas, path, pos, size) {
    ...
    for (var i=this.children.length-1; i>=0; i--) {
        ...
        $pos_KJX8__find_active[0] = c.pos[0], $pos_KJX8__find_active[1] = c.pos[1];
        if (c.state&UIFlags.HAS_PAN) {
        }
        if (inrect_2d(mpos, $pos_KJX8__find_active, c.size)) {
            found = true;
  ...
  var $pos_KJX8__find_active=[0, 0];
  var $zero_hrME_build_draw_old=[0, 0];
  _es6_module.add_class(UIFrame);
  UIFrame = _es6_module.add_export('UIFrame', UIFrame);
}, '/dev/cleanfairmotion/src/ui/UIFrame.js');
es6_module_define('UIPack', ["mathlib", "UIFrame", "UIWidgets", "UIElement", "toolprops"], function _UIPack_module(_es6_module) {
...
}, '/dev/cleanfairmotion/src/ui/UIPack.js');
es6_module_define('UISplitFrame', ["UIPack", "UIElement", "UIFrame"], function _UISplitFrame_module(_es6_module) {
  var UIFrame=es6_import_item(_es6_module, 'UIFrame', 'UIFrame');
  var UIElement=es6_import_item(_es6_module, 'UIElement', 'UIElement');
  var UIFlags=es6_import_item(_es6_module, 'UIElement', 'UIFlags');
  var PackFlags=es6_import_item(_es6_module, 'UIElement', 'PackFlags');
...
}, '/dev/cleanfairmotion/src/ui/UISplitFrame.js');
es6_module_define('icon', [], function _icon_module(_es6_module) {
  "use strict";
  var $ret_xHDp_enum_to_xy;
  var IconManager=_ESClass("IconManager", [function IconManager(gl, sheet_path, imgsize, iconsize) {
    ...
    x*=cellsize[0];
    y*=cellsize[1];
    $ret_xHDp_enum_to_xy[0] = x;
    $ret_xHDp_enum_to_xy[1] = y;
    return $ret_xHDp_enum_to_xy;
  ...
  var $ret_xHDp_enum_to_xy=[0, 0];
  _es6_module.add_class(IconManager);
  IconManager = _es6_module.add_export('IconManager', IconManager);
  var icon_vshader="\n\n";
  var icon_fshader="\n";
}, '/dev/cleanfairmotion/src/ui/icon.js');
es6_module_define('UIWidgets', ["toolprops", "../editors/viewport/events", "mathlib", "units", "UIElement", "UIFrame"], function _UIWidgets_module(_es6_module) {
  var $_mh;
  var $_swapt;
  var UIFrame=es6_import_item(_es6_module, 'UIFrame', 'UIFrame');
  ...
  var $pos_G_GE_build_draw;
  var $high_clr_71cU_build_draw;
  var $size_XghQ_build_draw;
  var $inset_clr_AiUn_build_draw;
  var UIButtonIcon=_ESClass("UIButtonIcon", UIButton, [function UIButtonIcon(ctx, text, icon, pos, size, path, callback, hint, use_small_icon) {
    ...
    if (this.icon==-1) {
        if (!(this.state&UIFlags.ENABLED))
          ...
          if (this.clicked)
          canvas.box($pos_G_GE_build_draw, $size_XghQ_build_draw, uicolors["IconInv"]);
        else 
          if (this.state&UIFlags.HIGHLIGHT)
          canvas.box($pos_G_GE_build_draw, $size_XghQ_build_draw, uicolors["HighlightIcon"]);
        else 
          canvas.box($pos_G_GE_build_draw, this.size, uicolors["IconBox"]);
        return ;
    }
    var pad=this.pad;
    var isize=this.small_icon ? canvas.iconsheet16.cellsize : canvas.iconsheet.cellsize;
    if (isize[0]>this.size[0])
      $pos_G_GE_build_draw[0] = 1;
    else 
      $pos_G_GE_build_draw[0] = 1;
    $pos_G_GE_build_draw[1] = 0;
    var $size_XghQ_build_draw=this.size;
    if (this.bgmode=="button") {
      ...
      if (this.bgmode=="flat") {
        if (!(this.state&UIFlags.ENABLED))
          ...
          if (this.clicked)
          canvas.box($pos_G_GE_build_draw, $size_XghQ_build_draw, $inset_clr_AiUn_build_draw);
        else 
          if (this.state&UIFlags.HIGHLIGHT)
          canvas.box($pos_G_GE_build_draw, $size_XghQ_build_draw, $high_clr_71cU_build_draw);
    }
    if ($size_XghQ_build_draw[0]>isize[0])
      $pos_G_GE_build_draw[0]+=($size_XghQ_build_draw[0]-isize[0])*0.5;
    if ($size_XghQ_build_draw[1]>isize[1])
      $pos_G_GE_build_draw[1]+=($size_XghQ_build_draw[1]-isize[1])*0.5;
    if (this.small_icon)
      canvas.icon(this.icon, $pos_G_GE_build_draw, 0.75, true);
    else 
      canvas.icon(this.icon, $pos_G_GE_build_draw, 0.75, false);
    canvas.end(this);
  }]);
  var $pos_G_GE_build_draw=[0, 0];
  var $high_clr_71cU_build_draw=[0.9, 0.9, 0.9, 0.2];
  var $size_XghQ_build_draw=[0, 0];
  var $inset_clr_AiUn_build_draw=[0.3, 0.3, 0.3, 0.2];
...
}, '/dev/cleanfairmotion/src/ui/UIWidgets.js');
...
}, '/dev/cleanfairmotion/src/editors/viewport/selectmode.js');
es6_module_define('UITextBox', ["UIWidgets", "UIPack", "UIFrame", "UIHTMLTextBox", "UIElement", "mathlib", "../editors/viewport/events"], function _UITextBox_module(_es6_module) {
  var $_mh;
  var $_swapt;
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var inrect_2d=es6_import_item(_es6_module, 'mathlib', 'inrect_2d');
...
}, '/dev/cleanfairmotion/src/ui/UITextBox.js');
...
}, '/dev/cleanfairmotion/src/ui/UIHTMLTextBox.js');
es6_module_define('ScreenKeyboard', ["UIElement", "../editors/viewport/events", "UIWidgets", "UIFrame", "UIPack"], function _ScreenKeyboard_module(_es6_module) {
  var PackFlags=es6_import_item(_es6_module, 'UIElement', 'PackFlags');
  var KeyMap=es6_import_item(_es6_module, '../editors/viewport/events', 'KeyMap');
  var ToolKeyHandler=es6_import_item(_es6_module, '../editors/viewport/events', 'ToolKeyHandler');
  var FuncKeyHandler=es6_import_item(_es6_module, '../editors/viewport/events', 'FuncKeyHandler');
...
}, '/dev/cleanfairmotion/src/ui/ScreenKeyboard.js');
...
}, '/dev/cleanfairmotion/platforms/common/platform_api.js');
...
}, '/dev/cleanfairmotion/platforms/common/platform_capabilies.js');
es6_module_define('platform_utils', [], function _platform_utils_module(_es6_module) {
}, '/dev/cleanfairmotion/platforms/common/platform_utils.js');
es6_module_define('platform', ["platform_html5", "theplatform", "config", "../src/wasm/load_wasm", "platform_chromeapp", "platform_phonegap"], function _platform_module(_es6_module) {
  var config=es6_import(_es6_module, 'config');
...
}, '/dev/cleanfairmotion/platforms/platform.js');
es6_module_define('UIMenu', ["UIWidgets", "../editors/viewport/events", "UIPack", "mathlib", "toolops_api", "UIElement", "UITextBox", "UIFrame"], function _UIMenu_module(_es6_module) {
  var UIElement=es6_import_item(_es6_module, 'UIElement', 'UIElement');
  var PackFlags=es6_import_item(_es6_module, 'UIElement', 'PackFlags');
  var UIFlags=es6_import_item(_es6_module, 'UIElement', 'UIFlags');
  var CanvasFlags=es6_import_item(_es6_module, 'UIElement', 'CanvasFlags');
...
}, '/dev/cleanfairmotion/src/ui/UIMenu.js');
es6_module_define('RadialMenu', ["UIElement", "UIPack", "mathlib", "../editors/viewport/events", "toolops_api", "UIWidgets", "UITextBox", "UIFrame"], function _RadialMenu_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var aabb_isect_2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_2d');
  var inrect_2d=es6_import_item(_es6_module, 'mathlib', 'inrect_2d');
  var UIElement=es6_import_item(_es6_module, 'UIElement', 'UIElement');
...
}, '/dev/cleanfairmotion/src/ui/RadialMenu.js');
es6_module_define('UIWidgets_special', ["mathlib", "UIPack", "UIElement", "../editors/viewport/events", "colorutils", "UIWidgets", "UIMenu", "UITextBox", "UIFrame"], function _UIWidgets_special_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var inrect_2d=es6_import_item(_es6_module, 'mathlib', 'inrect_2d');
  var aabb_isect_2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_2d');
  var UIElement=es6_import_item(_es6_module, 'UIElement', 'UIElement');
  ...
  var $ret_Lozh=undefined;
  function get_editor_list() {
    if ($ret_Lozh==undefined) {
        $ret_Lozh = new GArray();
  ...
  var $pos_HiKc_do_mouse;
  var $mpos__Ru2_do_mouse;
  var $sz_ADcw_build_draw;
  var $v1_AbyE_build_draw;
  var $v3_RC8F_build_draw;
  var $clr_dsVG_build_draw;
  var $pos1_pu6s_build_draw;
  var $size_aPZG_do_mouse;
  var $v2_pd7r_build_draw;
  var $v4_atUc_build_draw;
  var UIColorField=_ESClass("UIColorField", UIElement, [function UIColorField(ctx, callback) {
    ...
    $mpos__Ru2_do_mouse[0] = event.x;
    $mpos__Ru2_do_mouse[1] = event.y;
    if (this.mode=="h") {
        this.h = ($mpos__Ru2_do_mouse[0]-7)/(this.size[0]-12-2);
      ...
      if (this.mode=="sv") {
        var v=$mpos__Ru2_do_mouse[0]/this.size[0];
        v = Math.sqrt(v);
        var s=($mpos__Ru2_do_mouse[1]-this.huehgt+2)/(this.size[1]-this.huehgt);
    ...
    var cs=_hue_field;
    var segs=cs.length;
    var wid=Math.ceil((this.size[0]-2-$sz_ADcw_build_draw[0])/cs.length);
    var h=this.h, s=this.s, v=this.v;
    var halfx=Math.floor($sz_ADcw_build_draw[0]*0.5);
    ...
    for (var i=0; i<segs; i++) {
        ...
        $v1_AbyE_build_draw[0] = i*wid+1+halfx;
        $v1_AbyE_build_draw[1] = 1;
        $v2_pd7r_build_draw[0] = i*wid+1+halfx;
        $v2_pd7r_build_draw[1] = y;
        $v3_RC8F_build_draw[0] = i*wid+wid+1+halfx;
        $v3_RC8F_build_draw[1] = y;
        $v4_atUc_build_draw[0] = i*wid+wid+1+halfx, $v4_atUc_build_draw[1] = 1;
        canvas.quad($v2_pd7r_build_draw, $v3_RC8F_build_draw, $v4_atUc_build_draw, $v1_AbyE_build_draw, c1, c2, c3, c4, true);
    }
    canvas.quad($v4_atUc_build_draw, $v3_RC8F_build_draw, [this.size[0]-1, y], [this.size[0]-1, 1], cs[0]);
    $v1_AbyE_build_draw[0] = 0;
    $v1_AbyE_build_draw[1] = y+2;
    $v2_pd7r_build_draw[0] = 0;
    $v2_pd7r_build_draw[1] = this.size[1];
    $v3_RC8F_build_draw[0] = this.size[0];
    $v3_RC8F_build_draw[1] = this.size[1];
    $v4_atUc_build_draw[0] = this.size[0];
    $v4_atUc_build_draw[1] = 27;
    ...
    if (t<0||t>1)
      t = 0;
    for (var i=0; i<3; i++) {
        $clr_dsVG_build_draw[i] = cs[h1][i]+(cs[h2][i]-cs[h1][i])*t;
    }
    c1 = [0, 0, 0, 1];
    c2 = [0, 0, 0, 1];
    c3 = $clr_dsVG_build_draw;
    c4 = [1, 1, 1, 1];
    canvas.colorfield([0, this.huehgt], [this.size[0], this.size[1]-this.huehgt], $clr_dsVG_build_draw);
    $pos1_pu6s_build_draw[0] = Math.floor(1+h*(this.size[0]-2-$sz_ADcw_build_draw[0]));
    $pos1_pu6s_build_draw[1] = Math.floor(y*0.5-$sz_ADcw_build_draw[1]*0.5);
    canvas.box($pos1_pu6s_build_draw, $sz_ADcw_build_draw);
    $pos1_pu6s_build_draw[0] = Math.floor((this.size[0]-$sz_ADcw_build_draw[0])*v*v);
    $pos1_pu6s_build_draw[1] = Math.floor((this.size[1]-y-4)*s+y+2-$sz_ADcw_build_draw[1]*0.5);
    canvas.box($pos1_pu6s_build_draw, $sz_ADcw_build_draw);
  }]);
  var $pos_HiKc_do_mouse=[0, 0];
  var $mpos__Ru2_do_mouse=[0, 0];
  var $sz_ADcw_build_draw=[12, 12];
  var $v1_AbyE_build_draw=new Vector2();
  var $v3_RC8F_build_draw=new Vector2();
  var $clr_dsVG_build_draw=[0, 0, 0, 1];
  var $pos1_pu6s_build_draw=[0, 0];
  var $size_aPZG_do_mouse=[0, 0];
  var $v2_pd7r_build_draw=new Vector2();
  var $v4_atUc_build_draw=new Vector2();
  _es6_module.add_class(UIColorField);
  UIColorField = _es6_module.add_export('UIColorField', UIColorField);
  var $white_fOJZ_build_draw;
  var $grey_0Xtu_build_draw;
  var UIColorBox=_ESClass("UIColorBox", UIElement, [function UIColorBox(ctx, color) {
    ...
    for (var i=0; i<tot; i++) {
        ...
        for (var j=0; j<tot2; j++) {
            var k=(i+j)%2;
            canvas.box2(pos, wid, k ? $white_fOJZ_build_draw : $grey_0Xtu_build_draw);
            pos[1]+=wid[1];
        }
        pos[0]+=wid[0];
    }
  ...
  var $white_fOJZ_build_draw=[1.0, 1.0, 1.0, 1.0];
  var $grey_0Xtu_build_draw=[0.3, 0.3, 0.3, 1.0];
  _es6_module.add_class(UIColorBox);
  UIColorBox = _es6_module.add_export('UIColorBox', UIColorBox);
  var $hsva_FuAL_update_widgets;
  var $hsva__c0J_hsv_callback;
  var UIColorPicker=_ESClass("UIColorPicker", RowFrame, [function UIColorPicker(ctx, color, color_length) {
    ...
    for (var i=0; i<$hsva_FuAL_update_widgets.length; i++) {
        if (isNaN($hsva_FuAL_update_widgets[i])) {
            console.log("Eek, NaN!");
            $hsva_FuAL_update_widgets[i] = 0.0;
        }
    }
    ...
    this.last_valid_hue = rgba_to_hsva(this._color, $hsva_FuAL_update_widgets, this.last_valid_hue);
    if ($hsva_FuAL_update_widgets[1]<0) {
        $hsva_FuAL_update_widgets[1] = this.last_valid_sat;
    }
    else {
      this.last_valid_sat = $hsva_FuAL_update_widgets[1];
    }
    this.field.h = $hsva_FuAL_update_widgets[0]*0.9999;
    this.field.s = $hsva_FuAL_update_widgets[1];
    this.field.v = $hsva_FuAL_update_widgets[2];
    ...
    if (isNaN(this.color[2])) {
        console.log("NaN!", this._color, $hsva_FuAL_update_widgets);
    }
    ...
    $hsva__c0J_hsv_callback[0] = h*0.9999;
    $hsva__c0J_hsv_callback[1] = s;
    $hsva__c0J_hsv_callback[2] = v;
    if (this.color_length==4)
      $hsva__c0J_hsv_callback[3] = this._color[3];
    else 
      $hsva__c0J_hsv_callback[3] = 1.0;
    this.last_valid_hue = h;
    this.last_valid_sat = s;
    this.last_valid_hue = hsva_to_rgba($hsva__c0J_hsv_callback, this._color, h*0.9999);
    this.update_widgets();
    this.on_colorchange();
  }]);
  var $hsva_FuAL_update_widgets=[0, 0, 0, 0];
  var $hsva__c0J_hsv_callback=[0, 0, 0, 0];
  ...
  var $zero_SMCD_build_draw;
  var $size2_CHb0_build_draw;
  var $one_n8r3_build_draw;
  var UIProgressBar=_ESClass("UIProgressBar", UIElement, [function UIProgressBar(ctx, value, min, max, min_wid, min_hgt) {
    ...
    canvas.box($zero_SMCD_build_draw, this.size, uicolors["ProgressBarBG"]);
    if (perc>0.0) {
        perc = Math.min(Math.max(0.0, perc), 1.0);
        $size2_CHb0_build_draw[1] = this.size[1]-2;
        $size2_CHb0_build_draw[0] = Math.floor(this.size[0]*perc)-2;
        canvas.box($one_n8r3_build_draw, $size2_CHb0_build_draw, uicolors["ProgressBar"]);
    }
    canvas.end(this);
  }]);
  var $zero_SMCD_build_draw=[0, 0];
  var $size2_CHb0_build_draw=[0, 0];
  var $one_n8r3_build_draw=[1, 1];
  ...
  var $pan_1DXr__vscroll_callback;
  var UIListBox=_ESClass("UIListBox", ColumnFrame, [function UIListBox(ctx, pos, size, callback) {
    ...
    $pan_1DXr__vscroll_callback[1] = value;
    this.listbox.velpan.set_pan($pan_1DXr__vscroll_callback);
...
}, '/dev/cleanfairmotion/src/ui/UIWidgets_special2.js');
es6_module_define('UITabPanel', ["UIWidgets", "UIElement", "UIPack", "mathlib", "UIFrame"], function _UITabPanel_module(_es6_module) {
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var inrect_2d=es6_import_item(_es6_module, 'mathlib', 'inrect_2d');
  var aabb_isect_2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_2d');
  var UIElement=es6_import_item(_es6_module, 'UIElement', 'UIElement');
...
}, '/dev/cleanfairmotion/src/ui/UITabPanel.js');
es6_module_define('utildefine', [], function _utildefine_module(_es6_module) {
  var $_mh;
  var $_swapt;
}, '/dev/cleanfairmotion/src/core/utildefine.js');
es6_module_define('dialog', ["UIElement", "UIWidgets", "UIPack", "UIFrame", "UICanvas", "UIWidgets_special", "UITextBox", "toolops_api"], function _dialog_module(_es6_module) {
  ...
  window.Dialog = Dialog;
}, '/dev/cleanfairmotion/src/ui/dialog.js');
es6_module_define('dialogs', ["toolprops", "UIWidgets", "UIWidgets_special", "svg_export", "toolops_api", "fileapi", "ajax", "strutils", "UIFrame", "UIPack", "spline_createops", "UITextBox", "dialog", "config", "UIElement"], function _dialogs_module(_es6_module) {
  var Dialog=es6_import_item(_es6_module, 'dialog', 'Dialog');
  var PackedDialog=es6_import_item(_es6_module, 'dialog', 'PackedDialog');
...
}, '/dev/cleanfairmotion/src/ui/dialogs.js');
es6_module_define('FrameManager', ["ScreenArea", "FrameManager_ops", "toolops_api", "UIElement", "config", "ScreenBorder", "dialogs", "struct", "UIPack", "UIFrame", "../editors/viewport/events", "mathlib", "UICanvas"], function _FrameManager_module(_es6_module) {
  "use strict";
  var config=es6_import(_es6_module, 'config');
  ...
  window.Area = Area;
}, '/dev/cleanfairmotion/src/windowmanager/FrameManager.js');
es6_module_define('FrameManager_ops', ["ScreenArea", "mathlib", "struct", "UICanvas", "UIPack", "UIElement", "../editors/viewport/events", "UIFrame", "dialogs", "toolops_api"], function _FrameManager_ops_module(_es6_module) {
  "use strict";
  var login_dialog=es6_import_item(_es6_module, 'dialogs', 'login_dialog');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var get_rect_lines=es6_import_item(_es6_module, 'mathlib', 'get_rect_lines');
...
}, '/dev/cleanfairmotion/src/windowmanager/FrameManager_ops.js');
es6_module_define('ScreenArea', ["UICanvas", "dialogs", "toolops_api", "UIWidgets", "../editors/viewport/events", "struct", "mathlib", "UIPack", "UIElement", "UIFrame", "UISplitFrame"], function _ScreenArea_module(_es6_module) {
  "use strict";
  var login_dialog=es6_import_item(_es6_module, 'dialogs', 'login_dialog');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var get_rect_lines=es6_import_item(_es6_module, 'mathlib', 'get_rect_lines');
...
}, '/dev/cleanfairmotion/src/windowmanager/ScreenArea.js');
es6_module_define('ScreenBorder', ["struct", "UIPack", "../editors/viewport/events", "UIFrame", "ScreenArea", "toolops_api", "dialogs", "mathlib", "UICanvas", "config", "FrameManager_ops", "UIElement"], function _ScreenBorder_module(_es6_module) {
  "use strict";
  ...
  var $black_ODby_on_mousemove;
  var $black_uch4_build_draw;
  var ScreenBorder=_ESClass("ScreenBorder", UIElement, [function ScreenBorder(area, borderindex) {
    ...
    this.canvas.clear();
    this.canvas.line([x, y], [sx, sy], $black_ODby_on_mousemove, $black_ODby_on_mousemove);
  }, function finish() {
    if (this.moving) {
        this.canvas.pop_layer();
        this.parent.pop_modal();
    ...
    if (this.moving)
      canvas.line(this.moving_line[0], this.moving_line[1], $black_uch4_build_draw, $black_uch4_build_draw);
  }]);
  var $black_ODby_on_mousemove=[0, 0, 0, 1];
  var $black_uch4_build_draw=[0, 0, 0, 1];
  _es6_module.add_class(ScreenBorder);
  ScreenBorder = _es6_module.add_export('ScreenBorder', ScreenBorder);
}, '/dev/cleanfairmotion/src/windowmanager/ScreenBorder.js');
...
}, '/dev/cleanfairmotion/src/editors/viewport/view2d_object.js');
