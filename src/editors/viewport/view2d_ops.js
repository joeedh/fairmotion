...
class ViewRotateZoomPanOp extends ToolOp {
  ...
  static tooldef() {
    return {
      toolpath: "view2d.viewrotatezoom", uiname: "View Rotate Zoom", is_modal: true, undoflag: UndoFlags.NO_UNDO,
      inputs  : {}, outputs: {}
    }
  }
  ...
  on_mousemove(event) {
    ...
    if (screen.tottouch === 2 && this.startids[2] !== undefined) this.transition("rotate");
  ...
  exec(ctx) {
    ...
    if (this.startids[2] === undefined) {
      ...
      if (v1.vectorDistance(v2) < 0.01) return;
  ...
  exec_pan(ctx) {
    ...
    if (isNaN(scenter[2])) scenter[2] = 0.0;
    ...
    if (isNaN(vec[0]) || isNaN(vec[1]) || isNaN(vec[2])) return;
  ...
  on_mouseup(event) {
    ...
    if (g_app_state.screen.tottouch === 0) this.end_modal();
  }
}
...
class ViewRotateOp extends ToolOp {
  ...
  static tooldef() {
    return {
      toolpath  : "view2d.orbit", uiname: "Orbit", is_modal: true, undoflag: UndoFlags.NO_UNDO, inputs: {
...
class ViewPanOp extends ToolOp {
  ...
  static tooldef() {
    return {
      toolpath  : "view2d.pan", inputs: {
        MV1: new Vec3Property(new Vector3(), "mvector1", "mvector1", "mvector1"),
        MV2: new Vec3Property(new Vector3(), "mvector2", "mvector2", "mvector2")
      }, outputs: {}, is_modal: true, undoflag: UndoFlags.NO_UNDO
    }
  }
  ...
  start_modal(ctx) {
    ...
    for (let v of ctx.mesh.verts) {
      if (isNaN(v.co[0]) || isNaN(v.co[1]) || isNaN(v.co[2])) continue;
