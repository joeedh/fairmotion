/*
 * Drives the running Electron app (`pnpm electron`) over the Chrome DevTools
 * Protocol, via Playwright's connectOverCDP.
 *
 ...
 *
 * `pnpm cdp eval "__fm.datapathCount()"` reaches the same debug bridge the
 * Playwright specs use, so an electron-only bug can be interrogated with the
 * same oracles as the html5 baseline.
 *
...
/**
 * Connect to the app's CDP endpoint and locate the app window's page.
...
export async function connectApp({port = 9222} = {}) {
  ...
  /* Devtools windows show up here too; prefer the app window. */
  const page = pages.find((p) => p.url().includes("index.html")) ?? pages[0];

  if (!page) {
    await browser.close();
    throw new Error(`No pages found on CDP port ${port}. Is \`pnpm electron\` running?`);
  }
...
async function main() {
  ...
  try {
    switch (cmd) {
      ...
      case "screenshot": {
        const file = rest[0] ?? "electron-screenshot.png";
