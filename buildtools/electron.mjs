/*
 * Launches the built electron target with the Chrome DevTools Protocol
...
if (!fs.existsSync(path.join(appDir, "fcontent", "app.js"))) {
  console.log("dist/electron not built; running build:electron...");

  const res = spawnSync(process.execPath, [path.join(root, "buildtools", "esbuild.mjs"), "--electron"], {
    cwd  : root,
    stdio: "inherit",
  });
