...
export type WidgetToolOpClass = {
  ...
  create_widgets(manager: ManipulatorManager, ctx: FullContext): Manipulator | undefined;
};
...
export class HandleBase {
  ...
  on_click(e: WidgetEvent, view2d: View2DHandler, id: string | number): boolean | undefined {
    return undefined;
  }
...
export class ManipHandle extends HandleBase {
  ...
  constructor(v1: Vector2, v2: Vector2, id: string | number, shape: number, clr?: number[]) {
    super();
    ...
    if (this.color.length === 3) this.color.push(1.0);
  ...
  render(canvas: HTMLCanvasElement, g: Canvas2D) {
    ...
    if (this.shape === HandleShapes.ARROW) {
      ...
      let dx = this.v2[0] - this.v1[0],
        dy = this.v2[1] - this.v1[1];
      let dx2 = this.v1[1] - this.v2[1],
        dy2 = this.v2[0] - this.v1[0];
...
export class ManipCircle extends HandleBase {
  ...
  constructor(p: Vector2 | number[], r: number, id: string | number, clr?: number[]) {
    super();
...
export class Manipulator {
  ...
  circle(p: Vector2 | number[], r: number, id: string | number, clr: number[] = [0, 0, 0, 1.0]) {
  ...
  on_click(event: WidgetEvent, view2d: View2DHandler) {
    return this.active !== undefined
      ? this.active.on_click(event, view2d, this.active.id)
      : undefined;
  }
}
...
export class ManipulatorManager {
  ...
  create(cls: WidgetToolOpClass, do_push = true, ctx: FullContext = this.ctx) {
    ...
    if (do_push) this.push(mn);

    return mn;
  }

  on_tick(ctx: FullContext) {
    if (this.active !== undefined && this.active.on_tick !== undefined) this.active.on_tick(ctx);
  }
