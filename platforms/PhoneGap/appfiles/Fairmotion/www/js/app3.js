es6_module_define('spline_base', ["toolprops", "mathlib", "struct", "eventdag"], function _spline_base_module(_es6_module) {
  ...
  var CustomDataSet=_ESClass("CustomDataSet", Array, [function CustomDataSet() {
    ...
    while ($srcs2_ZxnW_interp.length<srcs.length) {
      $srcs2_ZxnW_interp.push(0);
    }
    $srcs2_ZxnW_interp.length = srcs.length;
    for (var i=0; i<this.length; i++) {
        for (var j=0; j<srcs.length; j++) {
            $srcs2_ZxnW_interp[j] = srcs[j][i];
        }
        this[i].interp($srcs2_ZxnW_interp, ws);
    }
  }, function copy(src) {
    for (var i=0; i<this.length; i++) {
        this[i].copy(src[i]);
  ...
  var $flip_out_H_Zr__get_nextprev;
  var $ret_cache_iVCZ_global_to_local;
  var $_co_lYis_global_to_local;
  var $arr_NjMa_global_to_local;
  var $_vec_4j_M_global_to_local;
  var CurveEffect=_ESClass("CurveEffect", [function CurveEffect() {
    ...
    p = p._get_nextprev(donext, $flip_out_H_Zr__get_nextprev);
    var flip=$flip_out_H_Zr__get_nextprev[0];
    if (p==undefined) {
        return undefined;
    }
    while (i>0) {
    ...
    var co;
    if (fixed_s!=undefined) {
        $arr_NjMa_global_to_local[0] = this.evaluate(fixed_s);
        $arr_NjMa_global_to_local[1] = fixed_s;
        co = $arr_NjMa_global_to_local;
    }
    else {
    ...
    var n2=$_vec_4j_M_global_to_local.zero().load(p).sub(co).normalize();
    n1[2] = n2[2] = 0.0;
    ...
    var ret=$ret_cache_iVCZ_global_to_local.next();
    ret[0] = s;
    ret[1] = t;
    ret[2] = a;
    return ret;
  ...
  var $flip_out_H_Zr__get_nextprev=[0];
  var $ret_cache_iVCZ_global_to_local=cachering.fromConstructor(Vector3, 64);
  var $_co_lYis_global_to_local=new Vector3();
  var $arr_NjMa_global_to_local=[0, 0];
  var $_vec_4j_M_global_to_local=new Vector3();
...
}, '/dev/cleanfairmotion/src/curve/spline_base.js');
es6_module_define('spline_types', ["selectmode", "toolprops_iter", "eventdag", "toolprops", "spline_base", "struct", "mathlib", "config", "spline_multires", "spline_math"], function _spline_types_module(_es6_module) {
  "use strict";
  ...
  var SplineVertex=_ESClass("SplineVertex", SplineElement, [function SplineVertex() {
    ...
    $ret_Qsya_aabb[0].load(this);
    $ret_Qsya_aabb[1].load(this);
    return $ret_Qsya_aabb;
  }), function sethide(state) {
    if (state)
      this.flag|=SplineFlags.HIDE;
    else 
  ...
  var $ret_Qsya_aabb=[new Vector3(), new Vector3()];
  ...
  var SplineSegment=_ESClass("SplineSegment", SplineElement, [function SplineSegment(v1, v2) {
    ...
    $minmax_vsrc_update_aabb.reset();
    min.zero();
    max.zero();
    var co=this.evaluate(0);
    $minmax_vsrc_update_aabb.minmax(co);
    var ds=1.0/(steps-1);
    for (var i=0, s = 0; i<steps; i++, s+=ds) {
        var co=this.evaluate(s*0.999999999);
        $minmax_vsrc_update_aabb.minmax(co);
    }
    min.load($minmax_vsrc_update_aabb.min);
    max.load($minmax_vsrc_update_aabb.max);
    min[2] = max[2] = 0.0;
  }, function closest_point(p, mode, fast) {
    if (fast==undefined) {
        fast = false;
  ...
  var $minmax_vsrc_update_aabb=new MinMax(2);
  ...
  var SplineLoopPath=_ESClass("SplineLoopPath", [function SplineLoopPath(l, f) {
    ...
    while (1) {
      ...
      l = __ival_l.value;
      $cent_KXnY_update_winding.add(l.v);
    }
    $cent_KXnY_update_winding.mulScalar(1.0/this.totvert);
    var wsum=0;
    var __iter_l=__get_iter(this);
    var l;
    while (1) {
      ...
      l = __ival_l.value;
      wsum+=math.winding(l.v, l.next.v, $cent_KXnY_update_winding) ? 1 : -1;
    }
    this.winding = wsum>=0;
  }, function asArray() {
    var l=this.l;
  ...
  var $cent_KXnY_update_winding=new Vector3();
  ...
  var SplineFace=_ESClass("SplineFace", SplineElement, [function SplineFace() {
    ...
    while (1) {
      ...
      while (1) {
        ...
        l = __ival_l.value;
        $minmax_rsSt_update_aabb.minmax(l.v.aabb[0]);
        $minmax_rsSt_update_aabb.minmax(l.v.aabb[1]);
        $minmax_rsSt_update_aabb.minmax(l.s.aabb[0]);
        $minmax_rsSt_update_aabb.minmax(l.s.aabb[1]);
      }
    }
    this._aabb[0].load($minmax_rsSt_update_aabb.min);
    this._aabb[1].load($minmax_rsSt_update_aabb.max);
  ...
  var $minmax_rsSt_update_aabb=new MinMax(3);
...
}, '/dev/cleanfairmotion/src/curve/spline_types.js');
es6_module_define('spline_query', ["spline_multires", "selectmode"], function _spline_query_module(_es6_module) {
  ...
  var $_mpos_eqB7_findnearest_mres;
  var $_mpos_ec1z_findnearest_vert;
  var $_v_Tbts_findnearest_mres;
  var $_v_LWIi_findnearest_vert;
  var SplineQuery=_ESClass("SplineQuery", [function SplineQuery(spline) {
    ...
    var spline=this.spline;
    var actlayer=spline.layerset.active;
    mpos = $_mpos_eqB7_findnearest_mres.load(mpos), mpos[2] = 0.0;
    ...
    while (1) {
      ...
      while (1) {
        ...
        $_v_Tbts_findnearest_mres.load(mapco);
        $_v_Tbts_findnearest_mres[2] = 0.0;
        editor.project($_v_Tbts_findnearest_mres);
        var dis=$_v_Tbts_findnearest_mres.vectorDistance(mpos);
    ...
    var min=1e+17;
    var ret=undefined;
    mpos = $_mpos_ec1z_findnearest_vert.load(mpos), mpos[2] = 0.0;
    ...
    while (1) {
      ...
      $_v_LWIi_findnearest_vert.load(co);
      $_v_LWIi_findnearest_vert[2] = 0.0;
      editor.project($_v_LWIi_findnearest_vert);
      var dis=$_v_LWIi_findnearest_vert.vectorDistance(mpos);
      if (dis<limit&&dis<min) {
          min = dis;
          ret = v;
      }
  ...
  var $_mpos_eqB7_findnearest_mres=new Vector3();
  var $_mpos_ec1z_findnearest_vert=new Vector3();
  var $_v_Tbts_findnearest_mres=new Vector3();
  var $_v_LWIi_findnearest_vert=new Vector3();
  _es6_module.add_class(SplineQuery);
  SplineQuery = _es6_module.add_export('SplineQuery', SplineQuery);
}, '/dev/cleanfairmotion/src/curve/spline_query.js');
es6_module_define('spline_draw', ["mathlib", "spline_math", "spline_element_array", "config", "spline_draw_sort", "view2d_editor", "animdata", "spline_draw_new", "selectmode", "spline_types"], function _spline_draw_module(_es6_module) {
  ...
  var $r_9yBC_draw_spline=[[0, 0], [0, 0]];
  ...
  var $margin_zddq_redraw_element=new Vector3([15, 15, 15]);
  var $aabb_jz2F_redraw_element=[new Vector3(), new Vector3()];
  function redraw_element(e, view2d) {
    e.flag|=SplineFlags.REDRAW;
    $margin_zddq_redraw_element[0] = $margin_zddq_redraw_element[1] = $margin_zddq_redraw_element[2] = 15.0;
    if (view2d!=undefined)
      $margin_zddq_redraw_element.mulScalar(1.0/view2d.zoom);
    var e_aabb=e.aabb;
    $aabb_jz2F_redraw_element[0].load(e_aabb[0]), $aabb_jz2F_redraw_element[1].load(e_aabb[1]);
    $aabb_jz2F_redraw_element[0].sub($margin_zddq_redraw_element), $aabb_jz2F_redraw_element[1].add($margin_zddq_redraw_element);
    window.redraw_viewport($aabb_jz2F_redraw_element[0], $aabb_jz2F_redraw_element[1]);
  }
  redraw_element = _es6_module.add_export('redraw_element', redraw_element);
}, '/dev/cleanfairmotion/src/curve/spline_draw.js');
es6_module_define('spline_draw_sort', ["animdata", "spline_multires", "view2d_editor", "spline_element_array", "mathlib", "spline_math", "selectmode", "spline_types", "config"], function _spline_draw_sort_module(_es6_module) {
  var aabb_isect_minmax2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_minmax2d');
  var ENABLE_MULTIRES=es6_import_item(_es6_module, 'config', 'ENABLE_MULTIRES');
  var SessionFlags=es6_import_item(_es6_module, 'view2d_editor', 'SessionFlags');
  var SelMask=es6_import_item(_es6_module, 'selectmode', 'SelMask');
  ...
  var $lists_8gkm_sort_layer_segments=new cachering(function() {
    return [];
  }, 2);
  function sort_layer_segments(layer, spline) {
    var list=$lists_8gkm_sort_layer_segments.next();
    list.length = 0;
    var visit={}
...
}, '/dev/cleanfairmotion/src/curve/spline_draw_sort.js');
es6_module_define('spline', ["eventdag", "native_api", "config", "solver_new", "spline_math", "spline_types", "struct", "view2d_editor", "spline_draw", "toolops_api", "solver", "selectmode", "const", "spline_query", "spline_element_array", "spline_multires", "lib_api"], function _spline_module(_es6_module) {
  "use strict";
  var PI=Math.PI, abs=Math.abs, sqrt=Math.sqrt, floor=Math.floor, ceil=Math.ceil, sin=Math.sin, cos=Math.cos, acos=Math.acos, asin=Math.asin, tan=Math.tan, atan=Math.atan, atan2=Math.atan2;
  ...
  var RecalcFlags=es6_import_item(_es6_module, 'spline_types', 'RecalcFlags');
  var $debug_id_gen_5uc0_Spline;
  var $_internal_idgen_tX9o_Spline;
  var $ws_Z3oe_split_edge;
  var $lastco_lmW4_trace_face;
  var $srcs__PBJ_split_edge;
  var Spline=_ESClass("Spline", DataBlock, [function Spline(name) {
    if (name==undefined) {
        name = undefined;
    }
    DataBlock.call(this, DataTypes.SPLINE, name);
    this._debug_id = $debug_id_gen_5uc0_Spline++;
    ...
    this._internal_id = $_internal_idgen_tX9o_Spline++;
    this.drawlist = [];
    ...
    $srcs__PBJ_split_edge[0] = v1.cdata, $srcs__PBJ_split_edge[1] = v2.cdata;
    this.copy_vert_data(nv, v1);
    nv.cdata.interp($srcs__PBJ_split_edge, $ws_Z3oe_split_edge);
    this.resolve = 1;
    return ret;
  ...
  }, function select_none(ctx, datamode) {
    if (ctx===undefined) {
    ...
    var __iter_elist=__get_iter(this.elists);
    var elist;
    while (1) {
      ...
      var __iter_e=__get_iter(elist.selected.editable(ctx));
      var e;
      while (1) {
        var __ival_e=__iter_e.next();
        if (__ival_e.done) {
  ...
  var $debug_id_gen_5uc0_Spline=0;
  var $_internal_idgen_tX9o_Spline=0;
  var $ws_Z3oe_split_edge=[0.5, 0.5];
  var $lastco_lmW4_trace_face=new Vector3();
  var $srcs__PBJ_split_edge=[0, 0];
...
}, '/dev/cleanfairmotion/src/curve/spline.js');
...
}, '/dev/cleanfairmotion/src/curve/solver.js');
es6_module_define('spline_multires', ["spline_base", "binomial_table", "struct"], function _spline_multires_module(_es6_module) {
  ...
  var BoundPoint=_ESClass("BoundPoint", [function BoundPoint() {
    ...
    $p_GQ4P_recalc_offset[0] = this[0];
    $p_GQ4P_recalc_offset[1] = this[1];
    var sta=seg._evalwrap.global_to_local($p_GQ4P_recalc_offset, undefined, this.s);
    this.t = sta[1];
    this.a = sta[2];
  }, function toString() {
  ...
  var $p_GQ4P_recalc_offset=new Vector3([0, 0, 0]);
  ...
  var $sum_4tPn_evaluate;
  var $ks_s7lQ_evaluate;
  var MultiResEffector=_ESClass("MultiResEffector", CurveEffect, [function MultiResEffector(owner) {
    ...
    $sum_4tPn_evaluate.zero();
    var i=0;
    for (var p in this.mr.points(0)) {
        $ks_s7lQ_evaluate[i] = p.s;
        i++;
    }
    for (var p in this.mr.points(0)) {
        ...
        if (isNaN(w))
          continue;
        $sum_4tPn_evaluate[0]+=p.offset[0]*w;
        $sum_4tPn_evaluate[1]+=p.offset[1]*w;
    }
    for (var i=0; i<2; i++) {
    ...
    co.add($sum_4tPn_evaluate);
    return co;
  }]);
  var $sum_4tPn_evaluate=new Vector3();
  var $ks_s7lQ_evaluate=new Array(2000);
  ...
  var $_co_QIms_add_point;
  var $sta_9vov_recalc_worldcos_level;
  var MultiResLayer=_ESClass("MultiResLayer", CustomDataLayer, [function MultiResLayer(size) {
    ...
    for (var p in this.points(level)) {
        $sta_9vov_recalc_worldcos_level[0] = p.s;
        $sta_9vov_recalc_worldcos_level[1] = p.t;
        $sta_9vov_recalc_worldcos_level[2] = p.a;
        var co=seg._evalwrap.local_to_global($sta_9vov_recalc_worldcos_level);
        var co2=seg._evalwrap.evaluate($sta_9vov_recalc_worldcos_level[0]);
        p[0] = co[0];
        p[1] = co[1];
  ...
  var $_co_QIms_add_point=[0, 0];
  var $sta_9vov_recalc_worldcos_level=[0, 0, 0];
  ...
  var $ret_tDpC_decompose_id=[0, 0];
  function decompose_id(id) {
    var mul=(1<<24);
    var eid=Math.floor(id/mul);
    id-=eid*mul;
    $ret_tDpC_decompose_id[0] = eid;
    $ret_tDpC_decompose_id[1] = id;
    return $ret_tDpC_decompose_id;
  }
...
}, '/dev/cleanfairmotion/src/curve/spline_multires.js');
es6_module_define('solver_new', ["spline_math", "spline_base"], function _solver_new_module(_es6_module) {
  ...
  var $tan_mIOw_solve=new Vector3();
        ...
        while (i<plen&&i>=0) {
          ...
          for (var ci=0; ci<2; ci++) {
              ...
              if (seg2!=undefined) {
                ...
                var h=seg1.handle(v);
                $tan_mIOw_solve.load(h).sub(v).normalize();
                if (v==seg1.v2)
                  $tan_mIOw_solve.negate();
                var ta=seg1.derivative(s1, order).normalize();
                var _d=Math.min(Math.max(ta.dot($tan_mIOw_solve), -1.0), 1.0);
                var r=acos(_d);
...
}, '/dev/cleanfairmotion/src/curve/solver_new.js');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_base.js');
es6_module_define('vectordraw_canvas2d', ["config", "vectordraw_jobs_base", "vectordraw_jobs", "mathlib", "vectordraw_base"], function _vectordraw_canvas2d_module(_es6_module) {
  "use strict";
  var config=es6_import(_es6_module, 'config');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_canvas2d.js');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_stub.js');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_canvas2d_simple.js');
es6_module_define('vectordraw_svg', ["mathlib", "config", "vectordraw_base"], function _vectordraw_svg_module(_es6_module) {
  "use strict";
  var config=es6_import(_es6_module, 'config');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var VectorFlags=es6_import_item(_es6_module, 'vectordraw_base', 'VectorFlags');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_svg.js');
es6_module_define('vectordraw_canvas2d_jobs', [], function _vectordraw_canvas2d_jobs_module(_es6_module) {
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_canvas2d_jobs.js');
es6_module_define('vectordraw_jobs', ["eventmanager", "vectordraw_jobs_base", "config", "platform"], function _vectordraw_jobs_module(_es6_module) {
  "use strict";
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_jobs.js');
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw_jobs_base.js');
es6_module_define('vectordraw', ["vectordraw_base", "vectordraw_canvas2d", "vectordraw_stub", "vectordraw_svg"], function _vectordraw_module(_es6_module) {
  "use strict";
...
}, '/dev/cleanfairmotion/src/vectordraw/vectordraw.js');
es6_module_define('strokedraw', [], function _strokedraw_module(_es6_module) {
  "use strict";
}, '/dev/cleanfairmotion/src/vectordraw/strokedraw.js');
es6_module_define('spline_draw_new', ["animdata", "config", "spline_multires", "spline_types", "view2d_editor", "selectmode", "spline_math", "vectordraw", "mathlib", "vectordraw_jobs", "spline_element_array"], function _spline_draw_new_module(_es6_module) {
  "use strict";
  var aabb_isect_minmax2d=es6_import_item(_es6_module, 'mathlib', 'aabb_isect_minmax2d');
  var MinMax=es6_import_item(_es6_module, 'mathlib', 'MinMax');
  var ENABLE_MULTIRES=es6_import_item(_es6_module, 'config', 'ENABLE_MULTIRES');
...
}, '/dev/cleanfairmotion/src/curve/spline_draw_new.js');
...
}, '/dev/cleanfairmotion/src/license/license_api.js');
es6_module_define('license_electron', [], function _license_electron_module(_es6_module) {
  ...
  getHardwareKey = _es6_module.add_export('getHardwareKey', getHardwareKey);
}, '/dev/cleanfairmotion/src/license/license_electron.js');
...
}, '/dev/cleanfairmotion/platforms/Electron/theplatform.js');
...
}, '/dev/cleanfairmotion/platforms/html5/platform_html5.js');
es6_module_define('platform_phonegap', ["platform_api"], function _platform_phonegap_module(_es6_module) {
  ...
  app = _es6_module.add_export('app', app);
}, '/dev/cleanfairmotion/platforms/PhoneGap/platform_phonegap.js');
es6_module_define('platform_chromeapp', ["platform_api"], function _platform_chromeapp_module(_es6_module) {
  ...
  PlatCapab = _es6_module.add_export('PlatCapab', PlatCapab);
}, '/dev/cleanfairmotion/platforms/chromeapp/platform_chromeapp.js');
...
}, '/dev/cleanfairmotion/src/wasm/load_wasm.js');
es6_module_define('built_wasm', ["load_wasm"], function _built_wasm_module(_es6_module) {
  ...
  tryInitWasm();
}, '/dev/cleanfairmotion/src/wasm/built_wasm.js');
